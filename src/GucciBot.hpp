...
#define GB_BUILD_LABEL "2026-07-28-b (SLOPE-EXIT ATTEMPT: useFastLockDelta() no longer hardcodes fwAnalyzing->fast. Traced why -e (2026-07-04) didn't hold: REND (SLRenderer path) and CALC both funnel through GucciUpdater::runUpdates via the same CCScheduler::update hook, so their fast/slow choice SHOULD already agree -- but -e forced CALC to always-fast regardless of m_lockDeltaMode, while REND's natural condition (m_lockDelta && mode==Performance && isPlaying() && !gb->renderer.recording -- note gb->renderer is the OLD legacy Renderer, dead in current UI, NOT SLRenderer) only goes fast when mode==Performance. On mode==Accuracy (the in-class default), REND goes slow/4-substep and CALC stayed forced fast/1-substep: exact 'renderer works, Calculate doesn't' mismatch. Fix: CALC now returns mode==Performance too, same condition as REND, instead of unconditional true. NOT verified against real physics -- verify via existing [SLOPE] logger (guccibot_slope.log): run PLAY, CALC, and an actual Render on the same level/slope, diff fast= and steps= columns at the divergence frame across all three. If they now all agree and ys= still diverges, this hypothesis was wrong -- revert and look elsewhere, don't stack another guess on top.)"
