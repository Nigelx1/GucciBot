...
#define GB_BUILD_LABEL "2026-08-16-h (1.2: Compact Mode, per Nigel's yBot comparison. New small corner panel (drawCompactWindow, gui.cpp) as a third window skin alongside the normal and MegaHack ones -- record/play, macro name, TPS/speed (own staged apply fields, compactTempTickRate/compactTempGameSpeed, separate from the full Tools tab's so editing one doesn't leave stale text in the other), frame advance + step, and Noclip/Layout Mode/Show Hitboxes/No Mirror/Swap Player Inputs as plain toggles -- no tabs, no descriptions, just the controls. Reuses the exact same GucciEngine fields the full menu's tabs already read/write, nothing new added to the engine side. Positioned top-left using the same corner-anchor recipe as the existing always-on HUD overlays (displayGameplayHUD etc.) but keeps real input (those are NoInputs/NoNav, this can't be). Toggle via a new Settings switch or its own keybind (kb.compactMode), both wired the same way megaHackLook already is; reuses the exact same shown/anim open-close state as the other two skins, so the normal menu keybind still shows/hides it. Compiles clean, untested in-game -- the actual control set is my own judgment call from yBot's reference plus what seemed most useful mid-attempt, not a literal port, so it's worth Nigel's take on what's missing or unnecessary before calling this done.)"
