...
#define GB_BUILD_LABEL "2026-08-13-h (NEW: one-time real-pixel sprite dump for the .gmd decoration side-project. Every procedural-decoration attempt so far has failed for the same reason -- guessing object shape from rotation/scale statistics instead of actually seeing it (crosshatch grid, then jittered clusters, then a checkerboard 'tessellation' that turned out to be plain lines, then a zigzag that still didn't read right). This hooks PlayLayer::createObjectsFromSetupFinished same as the objectID dump, but for each not-yet-captured object ID renders its real sprite -- unrotated, unscaled, straight from the texture atlas via GameObject::getTexture()/getObjectTextureRect() into a fresh CCSprite, drawn once into a small CCRenderTexture, saved as a transparent PNG to <mod save dir>/sprite_dump/<id>.png. Only captures IDs not already on disk, so repeated playthroughs build up a real reference sheet. Uses CCRenderTexture::beginWithClear/end/saveToFile(kCCImageFormatPNG), a standard cocos2d offscreen-render pattern not previously used elsewhere in this codebase -- untested, first attempt. No gameplay/physics/UI changes. Compiles clean, untested in-game.)"
