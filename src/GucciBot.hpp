...
#define GB_BUILD_LABEL "2026-08-16-g (1.2 WIP: new JuiceBot theme (.juice extension, coral accent + dark teal bg/card, per Juice's own Bloosix icon colors), 9th entry in kThemePresets -- picking it, saving/loading a macro under it, the Saved Replays badge, and Settings' theme picker all just fall out of the existing preset-table-driven system. Also fixed a real pre-existing bug while touching this: the activeTheme/activePreset load-time clamps were both still hardcoded to a max of 5 from before Bam/Sexyy existed, so picking either of those two and restarting silently downgraded you back to GiddeyBot -- bumped to 8 (now 9 with JuiceBot). Every place that maps a theme to a file extension needed the same parallel addition (7 extension-array call sites across engine_core.cpp/brr_format.cpp/gui.cpp, 2 badge-color chains, the save-path ternary x3, the settings preset-index-to-enum chain, the per-theme macro set + its clear/populate/erase sites) -- all updated together. Didn't touch the various theme-specific flavor-text easter eggs (tab quotes, title bar name, etc.) -- several of those were already incomplete/Toosii-only for the EXISTING themes before this, so skipping them for JuiceBot isn't a new gap. about.md's theme table updated. Compiles clean, untested in-game.)"
