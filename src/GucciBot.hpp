...
#define GB_BUILD_LABEL "2026-08-24-i (Color tweaks to the two new themes. RomoBot: accent swapped hazard-red -> silver per Nigel, still Cowboys navy bg -- the 'based off his dui' wink now lives ONLY in the subtitle text, nothing left in the palette itself. GrizzleyBot: recolored to a 'Grizzley Gang GTA RP palette' per Nigel -- honest caveat, I don't have specific knowledge of that exact RP server/crew's real branding, so this is my best-guess interpretation (GTA's own most iconic, universally-recognized identity: neon money-green on near-black), not a verified match to something specific Nigel has in mind. Flagging plainly rather than presenting it as certain -- easy to redo if he's picturing something else. Both kThemePresets entries and their matching macro-list tag colors (gui.cpp's tagCol assignments, which were still on the old red/orange-red values) updated together so the saved-replay list tags stay visually consistent with the actual theme colors. Compiles clean, untested in-game.)"
