...
#define GB_BUILD_LABEL "2026-08-24-d (GUI reorg: Frame Window/Calculate gets its own top-level tab. Nigel's complaint, audited and confirmed: Frame Window Tracker was ~432 of Macro->Hacks's ~560 lines (77%) -- algorithm settings, sweep/slack/debug, Manual Frame Windows editor, the full Tier system -- buried two clicks deep in a sub-tab literally named Hacks, while Survival Indicator (129 lines, far less actively developed) had its own top-level tab. Moved that content into a new top-level 'Frame Windows' tab (drawHacksTab renamed to drawFrameWindowsTab); added a second Calculate trigger button there per Nigel's ask ('put it in both') alongside the existing one in Macro->Replay. Also fixed a real mislabeling found during the audit: a 'Practice Range' header had ~10 Frame Window tuning controls (Max Window/Sweep/Slack/Position Tolerance/Debug Mode/etc) rendering under it after the one actual Practice Range toggle -- split into its own correctly-named 'Analysis Settings' header. Per Nigel's second ask, combined Macro's Tools and Hacks sub-tabs into one 'Tools & Hacks' (the real hack toggles -- Safe Mode/Trajectory/Hitboxes/Noclip/RNG Lock/Auto-Flip/Prevent Death/Mirror Inputs -- and Frame Stepping moved from the old Hacks sub-tab into Tools), which also resolves the separate naming collision the audit found (top-level Hacks tab vs Macro->Hacks sub-tab, same name unrelated content) since Macro->Hacks no longer exists as its own label. Mechanical tab-index changes (Jupiter/JMF's hardcoded activeTab==6 shifted to ==7 in 6 call sites, both parallel tab-bar arrays in drawTabBar/drawMegaHackWindow, the drawTabContent switch, sub-tab bar SN 3->2) all grepped for and updated together, verified none missed. Added a clamp on load for the now-smaller mainSubTab range so a value saved by an older build doesn't land on a dropped sub-tab and render blank -- same bug class as the old theme/preset-index clamps. Compiles clean, untested in-game -- pure GUI reorganization, no engine/physics logic touched, but the tab-index refactor is exactly the kind of mechanical change worth a real look-through before trusting blind.)"
