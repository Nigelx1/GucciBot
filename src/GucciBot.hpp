...
#define GB_BUILD_LABEL "2026-08-24-m (Second bass-shake drift fix -- Nigel: 'doesnt drift off the screen but it still drifts left' after -l, i.e. bounded now but still persistently creeping, a different bug than the unbounded random walk fixed last build. Root cause: -l's fix recovered the anchor every single frame by reading GetWindowPos() back and subtracting last frame's offset -- round-tripping window position through ImGui's own internal storage every frame while actively shaking. If that storage quantizes/rounds position to whole pixels for crisp rendering (typical for immediate-mode GUIs), each round-trip loses a small, consistently-signed fraction -- rounding only ever goes one way, never gains -- and that creeps steadily in one direction over hundreds of frames. Bounded per-frame, but a real cumulative drift over time, matching the report exactly. Fixed by no longer reading position back mid-episode at all: capture the anchor exactly ONCE when a shake episode starts (mirroring applyBigBrrrBounce's own restY pattern, which was already doing this correctly the whole time), then compute anchor+freshOffset directly every frame for the rest of the episode -- nothing left to round-trip while actively shaking. Compiles clean, untested in-game -- this is the second attempt at this specific bug, so it especially needs real confirmation before considering it done.)"
