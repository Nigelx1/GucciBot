...
#define GB_BUILD_LABEL "2026-08-14-c (Found the real ~80% opacity bug Nigel was actually asking about -- it was the BACKGROUND, not the decorative objects (those were already all 255). theme.bgColor's alpha was wired to theme.bgOpacity, a general user slider (default 0.96, range 0.5-1.0) meant for the normal translucent menu so gameplay shows through -- JMF inherited it instead of ignoring it. The real controlling call, ImGui::SetNextWindowBgAlpha(theme.bgOpacity*t), now uses a flat 1.f instead of theme.bgOpacity whenever jupiterActive. Also fixed the same issue in the MegaHack skin, which doesn't even use theme.bgColor for its background -- it has its own hardcoded near-black (bgMain/bgRail/bgHead, IM_COL32(18,19,26,...) etc) that was still showing through under the JMF backdrop regardless of theme; now forced to Nigel's navy at full 255 alpha when jupiterActive. Compiles clean, untested in-game.)"
