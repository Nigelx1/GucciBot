...
#define GB_BUILD_LABEL "2026-08-13-e (JMF backdrop take 4, per Nigel: grid was too weak and whole thing read as background not foreground. 1) Grid replaced: thin 12%-alpha crosshatch -> thick (2.5px) ~47%-alpha gold lines at wider spacing, with small solid gold triangle markers at intersections (matches the reference screenshots). 2) NEW: orb/road network -- 13 colour-coded ringed nodes (gold/cyan/pink/green/red) chained by thick gold roads, the boldest single motif from the reference shots and previously unbuilt. 3) NEW: 8 bold solid triangle spikes scattered as standalone decoration. 4) Opacity pushed up across the board: orbit-ring ornaments ~10-20%->~55-90%, stars ~18%->~67%, dust ~20-34%->~55-85%, skyline windows ~55-86%->~86-96%. Ornament stars now have a filled interior too, not just outline. Compiles clean, untested in-game.)"
