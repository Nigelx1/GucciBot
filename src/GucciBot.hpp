...
#define GB_BUILD_LABEL "2026-08-24-f (Tab bar text-overlap fix. Nigel sent a screenshot (in-game, Click Indicator tab open) showing 'Click Indicator' and 'Frame Windows' visually colliding in the top tab bar -- exactly the 'some text overlaps' he'd mentioned but not located yet. Real cause: drawTabBar divides the window width evenly across N tabs and centers each label in its column via CalcTextSize/AddText, but never clipped or wrapped it -- longer labels from this session's GUI reorg ('Click Indicator', 'Frame Windows') exceed a single column's width at the default window size and bleed into the neighbor. The Jupiter tab already had its own greedy word-wrap special-case for exactly this problem (its own long name), just never generalized -- extended that same wrap-to-fit-column logic to any tab label whose CalcTextSize exceeds its column, in both window skins (classic drawTabBar's horizontal columns and the MegaHack rail's vertical rows, which has the analogous risk of a label overflowing past its fixed 150px width even though its failure mode -- bleeding into the content pane rather than a neighboring tab -- wasn't the one actually reported). No tab renamed or reshuffled, purely a rendering fix. Compiles clean, untested in-game.)"
