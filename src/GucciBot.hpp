...
#define GB_BUILD_LABEL "2026-08-26-a (Comment cleanup, no functional change -- GWDdoS (new Write-access GitHub collaborator, Juice's recommendation) asked for LLM-narrative comments gone; Nigel's chosen middle ground was a controlled strip, not a blind one. Ran strip_comments.py (repo root) across all of src/ to remove every comment codebase-wide, verified compiles clean with zero comments first, then did a targeted pass re-adding short, plain comments ONLY at spots where forgetting the reasoning would concretely reintroduce a real bug: the deferred checkpoint-capture ordering (hook_playlayer.cpp/engine_updater.cpp), the async Task<T> lifetime pattern at all 3 file-picker call sites (gui.cpp/customtheme.cpp -- an unstored temporary here caused a real crash before), the frame-window slack sign (engine_core.cpp -- already flipped wrong once, produced inflated windows), the broken-objects checkpoint-neutralization mechanism (GucciBot.hpp -- was a silent no-op stub for a long time before), the Silicate field-name mappings (checkpoint_player.cpp -- deliberate, not typos), the Big Brrr shake anchor-drift fix (gui.cpp), and the theme-extension list duplication across 4+ files (customtheme.cpp). Everything else -- narrative history, who-said-what, dates, build numbers, ordinary explanatory comments without a concrete bug attached -- stayed stripped. Authorship credit (Claude wrote this codebase) is untouched and unaffected: it lives in about.md/README.md's Credits sections and every commit's Co-Authored-By trailer, never in inline code comments, so this doesn't conflict with Nigel's standing instruction to credit AI plainly. Compiles clean.)"
