...
#define GB_BUILD_LABEL "2026-08-13-g (NEW: one-time object-ID classifier dump for the .gmd decoration side-project, unrelated to JMF tab work. Hooks PlayLayer::createObjectsFromSetupFinished, walks m_objects, and appends any not-yet-seen {objectID, real GameObjectType enum value, level name} to <mod save dir>/guccibot_objectids.log -- authoritative since it reads the actual game's own m_objectType classification (Solid/Hazard/Decoration/etc per Geode/Enums.hpp) rather than guessing from a community spreadsheet. Only appends newly-seen IDs across runs, so playing a couple levels builds a running reference table. No gameplay/physics/UI changes. Compiles clean, untested in-game.)"
