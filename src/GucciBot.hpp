...
#define GB_BUILD_LABEL "2026-08-24-n (Create Your Own Theme -- full scope per Nigel: colors+identity+quotes, multiple saved/named themes, each with its own Big Brrr track+BPM+drop-offset. New THEME_CUSTOM enum sentinel (gui.hpp) since the custom list is runtime/unbounded, can't live in the fixed kThemePresets array. New CustomTheme struct + new file customtheme.cpp: real matjson serialization (Geode's own JSON lib, already transitively available, no new dependency) rather than copying BotSettingsPreset's existing hand-rolled-unescaped-string pattern, since quote text is freeform and very likely to contain literal quote characters that pattern can't safely handle. One JSON file per theme in customthemes/, mirroring BotSettingsPreset's one-file-per-preset convention otherwise. Threaded THEME_CUSTOM through every one of the ~15 places a theme gets read (found exhaustively via grep, same audit method as the Romo/Grizzley work): bot name/subtitle/brand-tag, all 3 flavor-quote slots + Credits badge, save-extension resolution (centralized into one new currentThemeExtension() helper instead of re-duplicating the 12-way ternary chain a 5th time), the 7 separate disk-scanning extension-list arrays (now allKnownMacroExtensions(), appending live custom extensions), macro-list tag+color (customThemeMacrosByExt map, since custom themes can't get one hardcoded unordered_set member each the way built-in themes do), and bigbrrr.cpp's BPM/offset/track selection. Full editor UI in Settings > Theme: name/subtitle/brand-tag/badge fields, 5 color pickers, 3 quote+attribution pairs, BPM/drop-offset inputs, and an mp3 import (staged during editing, committed to <ext>_brrr.mp3 only on Save, so a cancelled edit or a mid-edit crash can't corrupt or orphan a real theme's track). Fixed the active_theme load-clamp too -- THEME_CUSTOM would've been silently clamped into the built-in range on every restart otherwise, discarding 'you had a custom theme active' every single time. Compiled clean on the first full build attempt -- large surface area (5 files touched, 1 new), genuinely needs real in-game testing before trusting the editor flow, not just the compile.)"
...
public:
    ...
    // CustomTheme::extension) instead of theme name, since extension is
    // what's actually on disk and what list-UI tag lookups have on hand.
