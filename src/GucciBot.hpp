...
#define GB_BUILD_LABEL "2026-08-24-k (Bass-reactive menu shake, plus a quick RomoBot correction. Correction: badge line changed 'One Bad Night' -> 'One Bad Afternoon' -- Nigel: the actual DUI happened mid day, not at night. Main feature: Nigel's question from last build ('is there any way to vibrate the menu like a speaker on the bassy parts?') -- confirmed yes and built it, explicitly intense per Nigel ('it can be annoying on purpose if its accurate'). New FMOD DSP tap (BigBrrrManager::bassDspCallback, bigbrrr.cpp) attached directly to the Big Brrr playback channel (not the master group, so it only ever sees that track's own audio) -- passes audio through unchanged (unlike render/dsp.cpp's AudioRecorder, which silences its capture, fine for render-only use but wrong here since the user needs to actually hear the track), applies a cheap single-pole low-pass + RMS per buffer as a live bass-energy reading, written to an atomic<float> read from the main thread. gui.cpp: two new functions, bigBrrrFlickerAlpha (folded into the existing window-open alpha in both drawMainWindow/drawMegaHackWindow) and applyBigBrrrShake (violent position jitter, called alongside the existing applyBigBrrrBounce at both its call sites) -- squared response on both so quiet parts stay basically still and real hits actually hit. New 'Bass Shake' toggle in Settings > Fun, next to BIG BRRRR, off by default. Investigated true GPU blur first and ruled it out with a real reason, not just difficulty: ImGuiCocos (the vendored gd-imgui-cocos dependency) only exposes setup()/draw() callback registration, no render-target/backend hook to intercept for a post-process shader -- forking that dependency for one effect wasn't worth it, so shake+flicker is the actual deliverable, not a scaled-down consolation prize. Compiles clean, untested in-game.)"
