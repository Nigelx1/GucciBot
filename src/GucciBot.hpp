...
#define GB_BUILD_LABEL "2026-08-13-b (Jupiter tab: full decorative backdrop pulled from Nigel's 8 checkpoint screenshots, not just a colour swap. Hand-drawn (ImGui draw-list primitives, no ripped assets): a big orbit-ring-with-star ornament in the corner (concentric circles + a 5-point star), 8 scattered outline stars (4- and 5-point, mirroring the level's star clusters), a faint gold crosshatch grid across the whole tab, 8 drifting/pulsing cyan pixel-squares (sin-animated via ImGui::GetTime()), and a dark procedural skyline silhouette with lit windows along the bottom edge -- echoing the building-silhouette screenshots. All ambient layers sit at low alpha (drawn before the real widgets in the same window draw list) so nothing competes with actual UI content. Everything else from -a unchanged. Compiles clean, untested in-game.)"
