...
#define GB_BUILD_LABEL "2026-08-13-f (JMF backdrop take 5, per Nigel: still not landing, strip it back and go bolder on what's left. 1) REMOVED: crosshatch grid + triangle intersection markers entirely. 2) REMOVED: triangle spike decorations entirely. 3) REMOVED: orb/road network entirely (both the connecting road line -- which cut straight through the readable tab content -- and the ringed nodes themselves, whose mixed colours read as visually off). 4) ADDED: third orbit-ring 'wheel' ornament (top-center), joining the existing two. 5) Outline 'line star' shapes: 15 hand-placed -> 44 seeded-random, scattered across the full backdrop, alpha to fully opaque. 6) Opacity maxed on everything remaining: ornament rings/star fill/outline, dust squares, skyline building fill + lit windows all at or near 255 alpha. Compiles clean, untested in-game.)"
