...
#define GB_BUILD_LABEL "2026-08-24-e (GUI reorg round 2. Nigel: top-level Hacks tab's toggles (Hide Attempts/Auto Retry/Instant Respawn/Force Platformer) folded into the same 'Tools & Hacks' Macro sub-tab from the last build -- top-level Hacks tab removed. Click Sounds tab: confirmed first (grepped triggerClickAudio's actual call site, GJBaseGameLayer::handleButton -- fires on every real button press, not just during rendering) that click sounds play live during normal gameplay, not just baked into renders, before applying Nigel's own rule for it ('unless we can play them live, if so put them in hacks or sum') -- moved into 'Tools & Hacks' too rather than Render. Survival Indicator stays its own dedicated top-level tab per Nigel ('there should be a whole separate tab for click indicators and all that stuff') -- renamed 'Indicators' to 'Click Indicator' for clarity, content unchanged. Both moved tabs' function bodies (drawClicksTab/drawMoreHacksTab) left completely untouched -- just called directly from the end of drawToolsTab instead of duplicating ~150 lines of widget code. Top-level tab count: 12 -> 10 (Macro/Render/Autoclicker/Click Indicator/Frame Windows/JMF/Trainer/HUD/Settings/Credits) -- JMF's hardcoded Jupiter-takeover index moved 7->5 in the same 6 call sites as last build's 6->7 move, re-grepped and re-verified none missed. Also fixed a real, previously-unreported bug while in there: leaving the JMF tab snapped the window to the top-left corner instead of staying centered -- root cause was Jupiter's full-viewport takeover force-setting window position every frame, permanently overwriting ImGui's remembered position with nothing to ever re-center it afterward (windowPosInitialized only ever guarded the very first launch). switchTab() now resets that flag when leaving JMF specifically, in both window skins (classic + MegaHack rail, which needed its own equivalent fix since it used a different ImGuiCond_FirstUseEver-based mechanism). Compiles clean, untested in-game.)"
