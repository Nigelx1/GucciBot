...
#define GB_BUILD_LABEL "2026-08-14-e (JMF take 11, Nigel's most precise spec yet -- exact instructions, not photos, for the sun and wave ribbon. 1) Stars: 4-pointed ones removed, 5-pointed only, each with a plain (unfilled) regular pentagon outline nested in the middle. 2) Sun ornament fully rebuilt per exact spec: zero fill/opacity tricks, just plain ring outlines (5 layers), lines radiating from center through and past the outer ring, and tiny filled 'ball bearing' dots filling the gaps between consecutive rings. 3) Wave ribbon fully rebuilt per exact spec: a simple 3-segment \\/\\  zigzag, copied and shifted to the right, connected by a dense set of connector lines sampled along both paths -- since the copy is a constant offset of the original, every connector comes out exactly parallel by construction. 4) Buildings: added stepped rooftops on every third building for silhouette variety, still from memory of earlier reference shots pending a fresh building-specific photo. Compiles clean, untested in-game.)"
