...
#define GB_BUILD_LABEL "2026-08-19-k (Build -j's Lock Delta fix did NOT fix the checkpoint frame drift -- Juice retested and pinned down the actual mechanism directly: a checkpoint or click made on what the bot reads as frame N actually happens on frame N+1, confirmed with real before/after frame numbers (77->78 for a click, 65->66 for a checkpoint). This is a +1 correction at the specific hooks that read getFrame() BEFORE that tick's own frame increment (frameUpdateMidhook), not a general getFrame() change. Fixed at three call sites: hook_gjbasegamelayer.cpp's addInputToReplay (live click recording, handleButton's native input callback), hook_playlayer.cpp's storeCheckpoint (native checkpoint-touch callback), and engine_updater.cpp's earlyUpdateMidhook (backwards-stepping's own periodic checkpoint capture) -- this last one confirmed by the two hooks' actual registered offsets (earlyUpdate @ 0x237E42 fires before frameUpdate @ 0x238BAA), not just analogy. Deliberately did NOT touch engine_core.cpp's Capturing-pass checkpoint capture inside fwTick() -- that one reads getFrame() downstream of frameUpdateMidhook's own increment within the same call, already correctly timed. Also fixed a real inconsistency this surfaced: GucciPracticeFix::saveCurrent stored two different frame values for the same checkpoint (state.m_frameOffset from the caller's parameter, but m_storedFrames' StoredFrame::frame from an independent, unadjusted getFrame() re-read) -- now both derive from the same parameter, so the +1 fix actually reaches m_storedFrames (used by backstepping and Calculate's own checkpoint restore), not just m_savedCheckpoints (used by normal checkpoint respawns). Compiles clean, untested in-game -- this is grounded in Juice's own confirmed numbers, not my own guess, but still needs him to verify it actually closes the gap.)"
