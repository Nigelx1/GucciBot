...
#define GB_BUILD_LABEL "2026-07-28-c (SLOPE-EXIT: -b's fast/slow fix is CONFIRMED NOT the cause -- Nigel's [SLOPE] data showed steps=4/fast=0 identical between CALC and REND at f=1718, yet ys still diverges (3.000 vs 4.734). Checkpoint/probe-restart theory also ruled out: the whole CALC capture is one linear pass, zero backward frame jumps, so beginProbeRun() never even ran before the divergence. New lead: TrajectoryPreviewBaseLayer's GJBaseGameLayer hooks (collisionCheckObjects/canBeActivatedByPlayer/playerTouchedRing/playerTouchedTrigger, trajectory.cpp) redirect to ghost-simulation handlers based ONLY on TrajectoryPredictionService::isActiveSimulation(), with no check on which player triggered the call -- if that flag is ever true while the REAL player's own collision is being processed (possible overlap if Show Trajectory / Survival Indicator is enabled), the real player's pad/trigger interaction could get silently misrouted. Added diagnostic-only logging: sim=/pv=/si= fields on every [SLOPE] line (guccibot_slope.log), and a [HIJACK] log (regular geode log, not the slope file) that fires if this ever actually happens to the real player. No fix yet -- this is purely instrumentation to confirm or rule out the hijack theory. Test with Show Trajectory and/or Survival Indicator ON during a CALC run over the same slope.)"
