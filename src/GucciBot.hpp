...
#define GB_BUILD_LABEL "2026-08-14-d (JMF take 10, from Nigel's second round of feedback -- he flagged that his own earlier asks for filled stars and an exact sun match made things worse, not me. 1) Stars reverted to outline-only (kept the bigger size). 2) Sun ornament left untouched pending a fresh reference photo -- no new visual data to improve precision on it this round. 3) Wave ribbon completely rebuilt: the dense-lattice version was a tangled mess because sharp spine turns made the cross-diagonals overlap. Now a gentler-turning spine plus a clean chain of small independent diamonds walked along it at fixed arc-length steps -- no cross-segment geometry to go wrong regardless of how the path bends. 4) Buildings rebuilt as real filled silhouettes (dark fill, gold outline, scattered lit gold windows) instead of empty outlined boxes, and now span most of the width (0.42 to 0.95) instead of a small cluster stopping short of the sun. Removed the now-unused drawFilledStar helper. Compiles clean, untested in-game.)"
