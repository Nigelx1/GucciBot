...
#define GB_BUILD_LABEL "2026-08-14-r (Segment Looping now actually auto-restarts, not just a reminder. The FIRST time you cross the loop-start segment after enabling it, calls pl->markCheckpoint() -- the exact same call your own checkpoint keybind makes, going through GucciBot's already-existing, already-working hook (storeCheckpoint/practiceFix.saveCurrent) -- so it's using the real checkpoint system as designed, not reconstructing physics state by hand. Deliberately places only ONE checkpoint per enable-cycle rather than re-placing every lap: saveCurrent() unconditionally appends to m_savedCheckpoints with no cap, and removeCheckpoint() always removes whichever is currently latest, so repeated auto-placement would either grow that list forever or risk deleting a checkpoint Nigel placed himself if he adds one after mine mid-loop. One-shot placement sidesteps both. This is the one piece from this whole session that touches shared checkpoint machinery -- everything else in the JMF batch (ghosts, click deviation, stats, scrub preview) reads data only. Compiles clean, untested in-game.)"
