...
#define GB_BUILD_LABEL "2026-08-13-k (Sprite dump take 4: PNG writing worked this time (135 files captured!) but the CONTENT was wrong -- some ids (1753, 211) showed multiple neighboring atlas sprites bled together, others (8, 468) came out blank. Root cause: I was manually computing the atlas crop via GameObject::getObjectTextureRect() + CCSprite::createWithTexture(texture, rect), which doesn't know about GD's atlas packing quirks (sprites stored pre-rotated for tighter packing, trim offsets, etc). Switched to GameObject::displayFrame() + CCSprite::createWithSpriteFrame(frame) instead -- that's cocos2d's own CCSpriteFrame object, which already carries the correct rect/rotated/offset metadata, so there's no manual rect math left to get wrong. Also now sized the capture canvas off the resulting sprite's real getContentSize() instead of the old (possibly-bogus) rect dimensions. Compiles clean, untested in-game.)"
