...
#define GB_BUILD_LABEL "2026-08-13-d (Jupiter/JMF theming take 3, per Nigel's screenshot feedback. 1) Renamed tab 'Jupiter'->'JMF' in both tab-bar implementations. 2) Window now takes over the FULL VIEWPORT (ImGui::GetMainViewport()->Pos/Size, ImGuiCond_Always) for as long as activeTab==JMF instead of staying a small centered box -- snaps back to the normal window instantly on switching tabs. 3) GucciBot identity fully suppressed while on this tab: drawTitleBar() (name/version/diamond icon) and drawStatusBar() (TPS/speed/tick + brand word) are both skipped entirely, not just recolored -- point was Nigel didn't want it obviously readable as a mod tool. 4) Removed the edge-bleed effect (drawJupiterBleed) since the window IS the viewport now, nothing to bleed onto -- that budget went into the backdrop instead: two orbit-ring ornaments (was one), 22 scattered stars (was 8), 20 drifting dust squares (was 8), wider crosshatch spacing tuned for screen scale, skyline windows now two-tone (gold+cyan). Drag-handle shimmer also skipped in fullscreen (nothing to drag). Compiles clean, untested in-game.)"
