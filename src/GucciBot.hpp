...
#define GB_BUILD_LABEL "2026-08-13-m (Sprite dump take 6: forcing color/opacity/blend in the last build had no visible effect either way (id=8 still rendered its natural gradient, unaffected; id=211 stayed blank) -- ruling out color/blend as the cause for the still-blank ids. New theory: some decoration pieces are base-sprite-plus-child-node (a separate glow layer), so a standalone clone of just the base displayFrame() is legitimately blank if the real visible content is on a child I wasn't capturing. This captures the LIVE GameObject itself (real children, real state) instead of a fresh clone -- temporarily zeroes its rotation/scale and centers it in the capture canvas, renders, then immediately restores its original transform so nothing about the actual level visually changes. Debug log now also records each object's child count. Compiles clean, untested in-game.)"
