...
#define GB_BUILD_LABEL "2026-08-14-f (JMF take 12, real reference photo of the 63.99% sun ornament finally in hand. 1) Buildings removed entirely -- no replacement, real content and the wave ribbon are the whole backdrop now. 2) Wave ribbon shifted right (spine x-fractions +0.08) so it clears the ##jmfConstrain content child, features stay fully to the left. 3) Sun ornament rebuilt from scratch as a half-dome per the reference: ring ARCS (top half only, anchored at the bottom-right baseline) instead of full circles, small dots sitting ON each ring at intervals (alternating with short radial tick marks) instead of filling the gaps between rings, and a solid filled gold core with bold triangular spike rays fanning upward -- not thin radiating lines. 4) Pentagon-star alignment bug fixed: the nested pentagon is now built directly from the star's own inner (concave) vertices (the odd-indexed points jupiterStarPoints already computes) instead of recomputing angles separately, which was off by half a step and never lined up. Compiles clean, untested in-game.)"
