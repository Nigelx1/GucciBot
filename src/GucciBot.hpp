...
#define GB_BUILD_LABEL "2026-08-19-r (Donut fill rebuilt a THIRD time, this time not depending on CCDrawNode's border/stroke parameter at all -- Juice's screenshots showed -q's 'three concentric strokes via drawPolygon's own border' still came out wrong on a triangle (yellow outside, black middle, filled yellow center), meaning that border/stroke parameter just isn't reliable for small pointed polygons, only smooth shapes. New approach: each of the three rings (outer black border, colored band, inner black border) is built from scratch as a set of simple convex quads between two boundary vertex lists at adjacent radii (fillRingBetween/drawDonutRing) -- a 4-point convex quad can't misrender regardless of how pointed the shape is, so this doesn't depend on any polygon-fill edge case at all. Applies uniformly to Circle/Polygon/Star now. Higher confidence in this one specifically because it sidesteps the thing that broke twice, rather than tuning parameters on the same unreliable primitive again -- but still genuinely unverified in-game, so still not claiming it's fixed until Juice confirms.)"
