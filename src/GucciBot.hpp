...
#define GB_BUILD_LABEL "2026-08-24-h (Two new themes: RomoBot and GrizzleyBot, plus their own Big Brrr tracks (big_brrr_romo.mp3 130bpm/16+11-30s drop, big_brrr_grizzley.mp3 98bpm/90+4-30s drop -- bigbrrr.cpp's Maybach-only boolean special case refactored into a real per-theme switch to fit both in cleanly). Full integration pass across every spot a theme touches (found by grepping every existing .maybach/THEME_MAYBACH occurrence first, not guessing at the list): enum (gui.hpp), color palette (kThemePresets), bot name/subtitle/brand-tag, 3 theme-conditional GucciQuote blocks (Replay/Tools/Credits) + Credits badge, 4 save-extension ternary chains, 2 macro-tagging unordered_sets (romoMacros/grizzleyMacros, GucciBot.hpp) wired through reloadMacroList/list-UI-tag/delete-cleanup, 6 separate hardcoded extension-list literals across gui.cpp/engine_core.cpp/brr_format.cpp (a real dupe-list problem this codebase already had before today, not introduced by this change), preset-picker index mapping, and getThemeExtension()'s switch. RomoBot's color palette is Cowboys navy/silver with a hazard-red accent -- a color-only nod to 'based off his dui' rather than anything explicit; kept the one actual wink to a single subtitle line ('Definitely not over the limit') and otherwise built the persona around his real, positive claim to fame (play-calling prediction) rather than centering the whole theme on the incident -- flagging this judgment call explicitly since Nigel gave the concept but not exact copy. GrizzleyBot has no such angle (none was specified) -- straightforward Detroit-steel palette + persona built on his real music (First Day Out, Activated), same register as every other existing theme. Compiles clean, untested in-game -- verified both mp3s actually made it into the packaged .geode via unzip -l.)"
