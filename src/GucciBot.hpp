...
#define GB_BUILD_LABEL "2026-08-13-c (Jupiter theming, take 2: Nigel's feedback was that it read as a themed box inside a normal app, not the whole app being the art. Fixed: the theme colour swap (gold/indigo/cyan) now applies at drawMainWindow/drawMegaHackWindow scope for as long as activeTab==Jupiter, so the title bar, tab bar, and status bar all pick it up too -- not just the tab's own content. The backdrop (orbit ring, stars, crosshatch, dust, skyline) is drawn once across the FULL window rect before any chrome, with both content-area child regions given a transparent ChildBg so it shows through everywhere instead of being occluded by an opaque tab-local fill (removed that fill, it was actively hiding the new full-window version). New: a soft gold/cyan glow bleeds ~50px past the window's actual border on the foreground draw list (unclipped, screen-space), plus 5 small stars slowly orbiting just outside the window -- genuinely spilling past the menu's edge, not contained by it. The Jupiter tab button itself now stays warm gold even when not the active tab, in both tab-bar implementations. Compiles clean, untested in-game.)"
