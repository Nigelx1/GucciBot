...
#define GB_BUILD_LABEL "2026-08-24-g (Click Indicator tab no longer hides its own settings. Nigel: 'the survival indicator shouldn't HAVE to be on for other green window settings and stuff' -- drawIndicatorsTab had an early return, `if(!engine->survivalIndicator)return;`, right after the master 'Enable Survival Indicator' toggle, which hid Style/Timing/Appearance/Sound/Calibration/Stats entirely whenever the toggle was off. That's exactly why his own screenshot from a few messages ago showed an almost-empty tab -- the toggle was off, so everything below it was never drawn. Checked before removing it: nothing in that block actually depends on the indicator being live right now, it's all stored preferences the indicator reads whenever it does run, plus Calibration, which is independently useful prep work. The one genuine runtime gate in there (Start Calibration disabled without PlayLayer::get()) was already separate and correct, left untouched. Removed the early return -- everything is now always visible/editable regardless of the master toggle. Compiles clean, untested in-game.)"
