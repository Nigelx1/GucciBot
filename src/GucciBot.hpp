...
#define GB_BUILD_LABEL "2026-07-28-d (SLOPE-EXIT: hijack theory test #2 -- first attempt logged sim=/pv=/si= into guccibot_slope.log, came back sim=0 always even with pathPreview/survivalIndicator on, but that logger samples from frameUpdateMidhook, a different hook than where TrajectoryPredictionService::rebuildPreview() actually runs (PlayLayer::postUpdate), so it likely can't observe the overlap even if one happens. [HIJACK] itself logs via log::info -> geode's console log, which isn't reachable as a file on this dev machine, so it was unverifiable either way. Fix: [HIJACK] now ALSO writes to its own dedicated file, guccibot_hijack.log (same directory as guccibot_slope.log), logging every time isActiveSimulation() is true during collisionCheckObjects/canBeActivatedByPlayer/playerTouchedRing/playerTouchedTrigger, whether it's the real player or the preview one -- so we can see directly whether the flag ever overlaps with these hooks at all, not just whether it hijacks the real player specifically. Still diagnostic only, no fix. Test again: Show Trajectory or Survival Indicator ON, run Calculate over the same slope.)"
