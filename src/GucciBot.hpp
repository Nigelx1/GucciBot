...
#define GB_BUILD_LABEL "2026-08-15-f (Third audit pass, whole-codebase scope, core engine/replay focus per Nigel's request. m_pathSamples (ground truth for Show Macro Path + Calculate) was silently corrupted three ways: (1) an off-by-one -- the capture write happened AFTER the frame counter incremented and used push_back, landing each sample one index earlier than its frame, while both readers (Calculate's force-block, macropath.cpp) correctly assume the documented index==frame contract; fixed by writing directly to samples[frame] (resize-as-needed) instead of appending. (2) checkpoint retries never truncated old samples, so a respawn replayed with different inputs kept the PREVIOUS attempt's stale ground truth past the checkpoint -- now resized to match exactly what onReset/beginResumeRecording already do to the action atom. (3) resize-on-write also self-heals the forward-gap case (capture resuming far past where it left off, e.g. after a Calculate pass) instead of permanently misaligning every later index. Also: Calculate's force-write block restored position/velocity/rotation/ground state but not m_isDashing, leaving it inconsistent with everything else being pinned; now restored too. Separately: loading a legacy-format BRR right after a GBR6 one left the OLD macro's path samples and trainer-best-X attached (legacy branch never touched them) -- now cleared. loadPathSamples() reserve()'d an untrusted count straight from the sidecar file with no cap, so a corrupted .path file could throw on startup and crash the game -- now capped to what the file could actually hold. The Jupiter GDR bootstrap seeded its raw file into the SHARED replays folder under the exact name a real user macro would have, so a same-named user macro could get renamed aside and the seed's converted copy mistaken for it and relocated/deleted in its place -- seed now uses a private, collision-proof stem. Jupiter attempt/PB/heatmap tracking (destroyPlayer + levelComplete) had no level or isPlaying() gate at all, so dying on ANY level or the bot finishing ANY macro polluted the Jupiter tab's session stats -- now scoped like every other Jupiter feature, via a shared isJupiterLevel() (moved out of jupiterghost.cpp so hook_playlayer.cpp can reuse it). handleButton's real-click hooks (TrajectoryPredictionService, CalibrationService) fired for bot/autoclicker-injected clicks too, since handleButton is the base game's single choke point -- now gated to !isPlaying() (triggerClickAudio deliberately left alone, it already has its own correct guard). The hand-rolled msgpack GDR importer could stack-overflow on a deeply nested/malicious file (recursion had no depth cap) -- capped at 64 levels; also added a missing isMap check on the \"inputs\" container matching its sibling validations, and brought gamemodeChar back in sync with slopeLog's inline mapping (was missing the swing case). Compiles clean, untested in-game -- these are deep, sensitive fixes in the exact P3-adjacent territory that's burned this project before, please verify for real: record a macro, place a checkpoint, die, retry with different inputs, then check Show Macro Path and run Calculate.)"
