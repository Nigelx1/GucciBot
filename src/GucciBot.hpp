...
#define GB_BUILD_LABEL "2026-08-24-j (GrizzleyBot recolored again, this time from a real reference. Nigel sent the actual Grizzley Gang Gaming YouTube banner -- the earlier neon-green GTA guess was wrong; real palette is grungy black/charcoal, bold true-red crest/paint strokes, bold white graffiti lettering, tan/brown bear mascot. Rebuilt the palette from that image directly: true red-orange accent (distinct from ButlerBot's cooler crimson-pink), neutral warm-gray charcoal bg (distinct from Butler's reddish-black undertone), white text, muted tan-brown textSecondary as a small bear-fur nod. Macro-list tag color updated to match. RomoBot: added more dui-flavored lines per Nigel, who gave context this session didn't have on hand -- it was well received and Romo joked about it himself afterward, not a heavy topic. Reworked the Tools-tab quote ('I've had worse rides') and the Credits closing quote ('every single one -- well, almost') and badge ('One Bad Night') to lean into it a bit more while staying in the same affectionate-roast register as every other theme's quotes (still 'probably'-tagged where invented, no graphic/explicit content, doesn't glorify the actual danger of drunk driving -- just knowing winks, matching the self-deprecating energy Nigel described). Compiles clean, untested in-game.)"
