...
#define GB_BUILD_LABEL "2026-08-24-l (Fixed bass-shake drift. Nigel confirmed the intensity was great but 'it just keeps moving to the left.' Real bug, found by tracing the actual math, not guessed: applyBigBrrrShake read GetWindowPos() -- which already carried LAST frame's random offset -- and added a NEW random offset on top of it every single frame. Not a jitter around a fixed point, an honest-to-god random walk with nothing ever subtracting the previous step back out, so it wandered exactly like the report described. applyBigBrrrBounce avoids this by remembering restY once and always setting position FROM that anchor; Y never visibly drifted here either, but only because bounce runs first and unconditionally resets Y every frame regardless of what shake left it at -- X has no such correcting force, so only X visibly wandered, matching Nigel's report precisely. Fix: remember exactly what was added last frame (lastSx/lastSy) and subtract it back out of the current position first to recover the true pre-shake anchor, THEN apply this frame's fresh offset from that anchor -- proven correct by induction (X_n = trueX + thisFrame's offset, no compounding), and self-correcting even if the window gets dragged mid-shake since it only cares about undoing its own last contribution, not tracking an absolute position. Also snaps fully back to the anchor (not just stops touching it) when the shake fades out, so it doesn't leave a small residual offset stuck in place. Compiles clean, untested in-game.)"
