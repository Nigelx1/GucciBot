...
#define GB_BUILD_LABEL "2026-08-14-b (JMF take 9, from Nigel's annotated screenshot: (1) tab bar itself now shows the full wrapped 'Nigel's Jupiter My Favourite Trainer' name on the active JMF tab/rail row, not just a separate content title -- word-wrapped to fit whichever width the tab occupies. (2) Stars rebuilt as solid fills (pentagon core + 5 triangle points, since a star polygon is concave and can't just be convex-filled) instead of outline-only, and roughly doubled in size. (3) Real tab content now boxed into a width-constrained child so it can't cross the wave-ribbon backdrop line. (4) Wave ribbon rebuilt as a dense diamond/chevron lattice (subdivided rail segments, both diagonals per cell) instead of a sparse skeletal zigzag, matching Nigel's reference photo. (5) 'Wheel' ornament rebuilt as a glowing sun -- layered filled halo, solid core, bold rings, radiating sundial ticks -- instead of plain thin ring outlines. Compiles clean, untested in-game.)"
