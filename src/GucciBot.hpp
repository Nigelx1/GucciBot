...
#define GB_BUILD_LABEL "2026-08-13-l (Sprite dump take 5: displayFrame() fixed the crop -- id=8 (spike) now renders as a perfect, correctly-cropped triangle, real proof the pipeline is right. But several decoration ids (211, 1201, 817, 1888, 1766, 1836, 1763, 1123) still render blank despite non-degenerate content sizes (e.g. 211 is a real 30x30, not a sliver like 1753/1757 turned out to be at contentSize (30,1)/(15,1)). Likely cause: these pieces render via GD's dynamic color-channel system and/or an additive glow blend function in the live game, neither of which a freshly detached CCSprite inherits -- so it's compositing an invisible color/blend against the transparent clear. Forced setColor(white)/setOpacity(255)/setBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA) on the capture sprite, same blend pattern already used successfully elsewhere in this codebase (framewindow.cpp, hitboxes.cpp, macropath.cpp, practicerange.cpp). Color fidelity doesn't matter for a shape reference. Compiles clean, untested in-game.)"
