...
namespace gucci {
    ...
    void ClickSoundManager::ensureChannelGroup() {
        if (!channelGroup) {
            FMODAudioEngine::sharedEngine()->m_system->createChannelGroup("ClickSounds",
                                                                          &channelGroup);
        }
    }
    ...
    static bool isSupportedAudio(const std::filesystem::path& p) {
        ...
        return ext == ".wav" || ext == ".ogg" || ext == ".mov";
    }
    ...
    static std::vector<float> resampleInterleavedAudio(std::vector<float> const& input,
        ...
        size_t targetFrames =
            static_cast<size_t>(std::ceil(static_cast<double>(sourceFrames) * ratio));
    ...
    template <class Rng>
    static ResolvedClickSound
    resolveClickSound(const ClickPack& pack, bool pressed, float softness, Rng& rng) {
        std::uniform_real_distribution<float> softDist(0.0f, 1.0f);

        if (pressed) {
            bool useSoft =
                !pack.softClicks.empty() && (pack.hardClicks.empty() || softDist(rng) < softness);
            if (useSoft) {
        ...
        bool useSoftRelease =
            !pack.softReleases.empty() && (pack.hardReleases.empty() || softDist(rng) < softness);
        if (useSoftRelease) {
    ...
    static void scanSubfolder(const std::filesystem::path& dir,
                              const std::vector<std::string>& possibleNames,
                              std::vector<std::string>& out) {
        out.clear();
        for (auto& name : possibleNames) {
            auto sub = dir / name;
            if (!std::filesystem::exists(sub) || !std::filesystem::is_directory(sub))
                continue;
    ...
    void
    ClickSoundManager::loadClickPack(const std::string& packName, ClickPack& target, bool isP2) {
        ...
        scanSubfolder(
            packDir, {"clicks", "hard", "hardclicks", "hardclick", "click"}, target.hardClicks);
    ...
    FMOD::Sound* ClickSoundManager::getCachedSound(const std::string& path) {
        ...
        if (system->createSound(path.c_str(), FMOD_CREATESAMPLE, nullptr, &sound) != FMOD_OK ||
            !sound)
            return nullptr;
    ...
    void ClickSoundManager::updatePendingClicks() {
        ...
        {
            std::lock_guard<std::mutex> lock(pendingClickMutex);
            auto ready = std::remove_if(
                pendingClicks.begin(), pendingClicks.end(), [&](const PendingClick& pending) {
                    if (pending.playAt > now)
                        return false;
    ...
    void ClickSoundManager::startBackgroundNoise() {
        stopBackgroundNoise();
        if (!backgroundNoiseEnabled || backgroundNoiseVolume <= 0.0f)
            return;

        ClickPack& pack = p1Pack;
        if (pack.noiseFiles.empty())
            return;
        ...
        if (system->createSound(
                path.c_str(), FMOD_CREATESAMPLE | FMOD_LOOP_NORMAL, nullptr, &bgNoiseSound) !=
                FMOD_OK ||
            !bgNoiseSound)
            return;
    ...
    std::vector<float> ClickSoundManager::decodeClickToRaw(const std::string& filePath,
    ...
    std::vector<float>
    ClickSoundManager::generateClickAudio(const std::vector<MacroAction>& actions,
                                          float tickRate,
                                          float duration,
                                          int sampleRate,
                                          int startTick,
        ...
        geode::log::info("generateClickAudio: tickRate={}, duration={:.3f}s, sampleRate={}, "
                         "startTick={}, actions={}, totalSamples={}",
                         tickRate,
                         duration,
                         sampleRate,
                         startTick,
        ...
        for (auto& action : actions) {
            ...
            if (file.empty() || volume <= 0.0f)
                continue;
            if (action.frame < startTick)
                continue;
            ...
            auto clickSamples = (cacheIt != decodedClickCache.end())
                                    ? cacheIt->second
                                    : decodeClickToRaw(file, sampleRate);
            ...
            if (firstClickTime < 0.0)
                firstClickTime = timeSec;
            lastClickTime = timeSec;
        }

        geode::log::info(
            "generateClickAudio: placed {} clicks, first={:.3f}s last={:.3f}s (duration={:.3f}s)",
            clicksPlaced,
            firstClickTime,
            lastClickTime,
            duration);
        ...
        if (backgroundNoiseEnabled && backgroundNoiseVolume > 0.0f && !p1Pack.noiseFiles.empty()) {
            ...
            auto noiseSamples = (cacheIt != decodedClickCache.end())
                                    ? cacheIt->second
                                    : decodeClickToRaw(p1Pack.noiseFiles[0], sampleRate);
