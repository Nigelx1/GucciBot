...
#define GB_BUILD_LABEL                                                                             \
    "2026-08-26-c (Folder reorg by feature, GWDdoS's ask (both namespace+folders, grouped by "     \
    "feature not layer) -- folder half only tonight, namespace half deliberately skipped, see "    \
    "below. 55 files moved from a flat src/ into core/ hooks/ gui/ audio/ analysis/ trainers/ "    \
    "hacks/ tools/ (render/ already existed). CMake needed zero changes (GLOB_RECURSE already "    \
    "discovers subfolders); did need every #include reference rewritten to its new src/-relative " \
    "path, done via a script rather than by hand across 40 affected files to not miss one. Real "  \
    "bug found by actually compiling, not assumed away: src/renderer.hpp (the legacy "             \
    "pre-SLRenderer "                                                                              \
    "class, deliberately left unmoved to avoid colliding with render/renderer.hpp) still broke "   \
    "render/renderer.cpp's build -- MSVC's quote-include search checks directories of "            \
    "currently-open "                                                                              \
    "files up the include stack before falling through to -I, so two files sharing the exact "     \
    "same "                                                                                        \
    "basename in different folders can still collide indirectly even without literally merging "   \
    "them. "                                                                                       \
    "Fixed by renaming the legacy one to legacy_renderer.{cpp,hpp} instead. Namespace half NOT "   \
    "attempted -- GWDdoS's own answer (\"why the fuck would u hook in a namespace\") correctly "   \
    "resolved the original blocking concern (keep $modify() hook classes in global scope, "        \
    "namespace "                                                                                   \
    "everything else), but threading that through 33 hook classes across 24 files, correctly "     \
    "distinguishing what stays global vs. what moves into namespace gucci, is real remaining "     \
    "scope "                                                                                       \
    "on its own -- next session's first task, not rushed tonight. Global-externs question still "  \
    "unanswered from earlier; GWDdoS said ignore it for now. Compiles, links, and packages "       \
    "clean. "                                                                                      \
