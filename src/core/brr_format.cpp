...
namespace gucci {
    ...
    static PlayerStateBundle readAnchorPlayerV3(std::vector<uint8_t> const& payload,
                                                size_t& position) {
        PlayerStateBundle state;
        ...
        try {
            ...
            if ((flags & BRR_FLAG_HAS_DEATH_FRAMES) != 0 && payloadPosition + 4 <= payload.size()) {
                ...
                if (payloadPosition + 4 <= payload.size()) {
                    ...
                    if (startCount <= 100) {
                        for (uint32_t i = 0;
                             i < startCount && payloadPosition + 4 <= payload.size();
                             ++i) {
    ...
    BRRMacro* BRRMacro::deserialize(std::vector<uint8_t> const& data) {
        ...
        if (version > BRR_FORMAT_VERSION) {
            log::warn("[BRR] Format version {} is newer than supported ({})",
                      version,
                      BRR_FORMAT_VERSION);
            return nullptr;
        }
    ...
    namespace ReplayStorage {
        ...
        static bool replayNameTaken(std::filesystem::path const& dir,
