...
static void savePathSamples(const fs::path& macroPath,
    ...
    for (auto const& s : samples) {
        ...
        uint8_t p1flags = (s.p1OnGround ? 1 : 0) | (s.p1UpsideDown ? 2 : 0) |
                          (s.p1Dashing ? 4 : 0) | (s.p1OrbDash ? 8 : 0) | (s.p1OrbNonDash ? 16 : 0);
    ...
    namespace gdrmsgpack {
        ...
        static bool readNumber(const std::vector<uint8_t>& b, size_t& i, double& out) {
            ...
            case 0xd2:
                if (i + 5 > b.size())
                    return false;
                out = (double)(int32_t)(((uint32_t)b[i + 1] << 24) | ((uint32_t)b[i + 2] << 16) |
                                        ((uint32_t)b[i + 3] << 8) | b[i + 4]);
                i += 5;
                return true;
            case 0xd3: {
...
std::vector<GucciEngine::DiffEntry> GucciEngine::diffMacros(const std::string& a,
    ...
    for (size_t i = 0; i < n && out.size() < kMaxDiffs; ++i) {
        ...
        if (A.tick != B.tick || A.actionType != B.actionType || A.isPressed() != B.isPressed() ||
            A.isPlayer2() != B.isPlayer2()) {
...
void GucciEngine::saveBotSettingsPreset(const std::string& name) {
    ...
    f << "{\"name\":\"" << p.name << "\""
      ...
      << ",\"lockDeltaMode\":" << p.lockDeltaMode << ",\"ssbFix\":" << (p.ssbFix ? "true" : "false")
...
void GucciEngine::fwTick() {
    ...
    case FwState::Capturing: {
        ...
        if (player1 && !fwProbeDied) {
            log::info("[CAP-F] f={} steps={} x={:.1f} y={:.1f} yvel={:.1f} ground={} respawn={} "
                      "tps={:.0f}",
                      frame,
                ...
                if (!fwClickSamples[fwXYIndex].release) {
                    ...
                    if (frame < samples.size()) {
                        ...
                        fwClickSamples[fwXYIndex].orbNonDash =
                            fwClickSamples[fwXYIndex].player2 ? gt.p2OrbNonDash : gt.p1OrbNonDash;
                    } else {
        ...
        while (fwCapIndex < fwClickSamples.size()) {
            ...
            log::info(
                "[CAP] createCheckpoint @ f={} (click {} @ f={})", frame, fwCapIndex, clickFrame);
            StoredFrame sf;
            sf.frame = frame;
            if (cp) {
                cp->retain();
        ...
        if (fwXYIndex >= fwClickSamples.size() && fwCapIndex >= fwClickSamples.size()) {
            log::info("[GucciBot] Frame-window: capture done — {} checkpoints", fwCapStack.size());
...
void GucciEngine::cancelAnalysis() {
    ...
    log::info("[GucciBot] Frame-window: analysis cancelled — {} result(s) kept", fwMarks.size());
}
