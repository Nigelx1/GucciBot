...
void MenuInterface::openCustomThemeEditor(const CustomTheme* existing) {
    ...
    copyBuf(
        cteQuoteToolsAttr, sizeof(cteQuoteToolsAttr), customThemeEditBuffer.quoteTools.attribution);
    copyBuf(
        cteQuoteCreditsText, sizeof(cteQuoteCreditsText), customThemeEditBuffer.quoteCredits.text);
...
void MenuInterface::drawCustomThemeEditorPopup() {
    ...
    if (!ImGui::BeginPopupModal("Custom Theme Editor", nullptr, ImGuiWindowFlags_NoSavedSettings))
        return;

    ImGui::TextColored(
        theme.getAccent(), "%s", customThemeEditIsNew ? "New Custom Theme" : "Edit Custom Theme");
    ImGui::Separator();
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Shown on the Replay tab, Tools tab, and at the end of Credits -- matches "
    ...
    if (stagedAudioExists) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("Where BIG BRRRR starts playback -- skips a slow intro the same way the "
    ...
    if (saveClicked && canSave) {
        ...
        if (!typedExt.empty()) {
            ...
            bool renamedName =
                !customThemeEditIsNew && customThemeEditOriginalName != customThemeEditBuffer.name;
