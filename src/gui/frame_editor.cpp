...
namespace gucci {
    ...
    static void feDrawSolidRect(ImDrawList* dl,
                                ...
                                bool border = true) {
        ImVec4 fill(
            theme.cardColor.x, theme.cardColor.y, theme.cardColor.z, theme.cardColor.w * alpha);
    ...
    void FrameEditor::openGDR(const std::string& name, BRRMacro* macro) {
        ...
        for (size_t i = 0; i < macro->inputs.size(); i++) {
            ...
            EditorInput ei;
            ei.frame = src.tick;
            ei.actionType = static_cast<int>(src.actionType);
            ei.player2 = src.isPlayer2();
            ei.pressed = src.isPressed();
            ei.stepOffset = src.stepOffset;
            ei.originalIndex = i;
    ...
    void FrameEditor::applyToBRR() {
        if (!cachedBRR) {
            log::error("[FrameEditor] applyToBRR: cachedBRR is null, cannot save");
            return;
        }
        ...
        for (size_t idx : order) {
            ...
            cachedBRR->inputs.push_back(ti);
        }

        cachedBRR->anchors.clear();
        cachedBRR->checkpoints.clear();
        cachedBRR->persist();
    ...
    void FrameEditor::applyToGDR() {
        ...
        for (size_t idx : order) {
            ...
            BRRInput in;
            in.tick = ei.frame;
            in.actionType = static_cast<uint8_t>(ei.actionType);
            in.setPlayer2(ei.player2);
            in.setPressed(ei.pressed);
            in.stepOffset = ei.stepOffset;
            cachedBRR->inputs.push_back(in);
        }

        cachedBRR->anchors.clear();
        AccuracyMode accMode = cachedBRR->accuracyMode;
        int anchorInt = cachedBRR->savedAnchorInterval;
    ...
    int32_t FrameEditor::frameAtPixel(float pixelX, float canvasOriginX) const {
        return static_cast<int32_t>(
            std::floor((pixelX - canvasOriginX) / pixelsPerFrame + scrollX));
    }
    ...
    int FrameEditor::hitTestSegment(ImVec2 mousePos,
                                    ImVec2 lanesOrigin,
                                    float lanesWidth,
                                    float lanesHeight,
                                    int& edgeOut) const {
                ...
                if (hit >= 0) {
                    ...
                    if (edge == -1) {
                        ...
                        dragReleaseOrigFrame =
                            (seg.hasRelease && seg.releaseIndex != seg.pressIndex)
    ...
    void FrameEditor::drawToolbar(MenuInterface& ui, ImVec2 origin, float width) {
        ...
        if (dirty) {
            dl->AddText(ImVec2(nameX - 16.0f, origin.y + 10.0f),
                        feToU32(ImVec4(1.0f, 0.8f, 0.2f, 1.0f)),
                        "*");
        }
    }
        ...
        for (int32_t f = minorStart; f <= vEnd; f += minorInterval) {
            ...
            if (isMajor) {
                ...
                if (x + textSize.x + 4.0f < origin.x + width) {
                    dl->AddText(
                        ImVec2(x + 3.0f, origin.y + 1.0f), ui.theme.getTextSecondaryU32(), buf);
                }
            }
        }
    }
    ...
    void FrameEditor::drawLanes(MenuInterface& ui, ImVec2 origin, float width, float height) {
        ...
        dl->AddText(
            ImVec2(labelX, origin.y + laneH + 2.0f), feToU32(feWithAlpha(p2Color, 0.4f)), "P2");
        if (ui.fontSmall)
            ImGui::PopFont();
        ...
        for (int i = 0; i < (int)segments.size(); i++) {
            ...
            ImVec4 fillCol =
                feWithAlpha(baseCol, isSelected ? 0.80f : (isHovered ? 0.68f : baseAlpha));
            dl->AddRectFilled(
    ...
    void FrameEditor::drawDetailBar(MenuInterface& ui, ImVec2 origin, float width, float height) {
        ...
        ImGui::PushStyleColor(ImGuiCol_FrameBg, feToU32(ImVec4(0.1f, 0.1f, 0.12f, 0.8f)));
        ImGui::PushStyleColor(ImGuiCol_Text, feToU32(ui.theme.textPrimary));
        bool goSubmitted = ImGui::InputText("##goToFrame",
                                            goToFrameBuf,
                                            sizeof(goToFrameBuf),
                                            ImGuiInputTextFlags_EnterReturnsTrue |
                                                ImGuiInputTextFlags_CharsDecimal);
