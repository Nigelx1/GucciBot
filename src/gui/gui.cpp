...
template <class T>
static T
loadSV(Mod* mod, std::string_view key, T def, std::initializer_list<std::string_view> legacy = {}) {
...
static void drawJupiterBackdrop(ImDrawList* dl, ImVec2 pos, ImVec2 size, float time) {
    ...
    for (auto const& s : stars) {
        ...
        dl->AddPolyline(
            pent.data(), (int)pent.size(), IM_COL32(252, 245, 80, 255), ImDrawFlags_Closed, 2.2f);
    }
}
...
namespace Widgets {
    ...
    bool ModuleCard(const char* name,
        ...
        if (*enabled)
            dl->AddRectFilled(
                pos, ImVec2(pos.x + 3, pos.y + h), theme.getAccentU32(0.85f), theme.cornerRadius);
        ...
        ImVec4 tc =
            lerpColor(ImVec4(0.22f, 0.22f, 0.22f, 1.f), withAlpha(theme.getAccent(), 0.75f), tt);
...
void MenuInterface::drawTitleBar() {
    ...
    std::string subStr =
        ...
        : (activeTheme == THEME_SAWEETIE) ? "v" MOD_VERSION "  -  Icy girl. Tap in, don't fall off."
        : (activeTheme == THEME_MAYBACH) ? "v" MOD_VERSION "  -  Huh. Maybach Music. Frame perfect."
...
void MenuInterface::drawStatusBar() {
    ...
    dl->AddText(
        ImVec2(wp.x + padX + 12, barY + (barH - ts.y) * 0.5f), theme.getTextSecondaryU32(), buf);
...
void MenuInterface::drawMegaHackWindow() {
    ...
    if (jupiterActive) {
        ...
        if (!windowPosInitialized) {
            ImGui::SetNextWindowPos(ImVec2(center.x - mhSize.x * 0.5f, center.y - mhSize.y * 0.5f),
                                    ImGuiCond_Always);
            windowPosInitialized = true;
        } else {
            ImGui::SetNextWindowPos(ImVec2(center.x - mhSize.x * 0.5f, center.y - mhSize.y * 0.5f),
...

    {
        ...
        if (engine->isRecording()) {
            ...
            if (ImGui::InputTextWithHint(
                    "##compactMacroName", "Macro name", macroNameBuffer, sizeof(macroNameBuffer)))
...
void MenuInterface::drawReplayTab() {
    ...
    if (auto* replayCustom = getActiveCustomTheme())
        ...
        Widgets::GucciQuote("\"Regular season replays don't count. I lock in for the playoffs.\"",
        ...
        Widgets::GucciQuote(
            "\"Every replay a hit. Every frame a boss move.\"", "-- Rick Ross, probably", theme);
    else if (activeTheme == THEME_ROMO)
        Widgets::GucciQuote(
            "\"I called that replay before it even happened.\"", "-- Tony Romo, probably", theme);
        ...
        Widgets::GucciQuote(
            "\"I got so many replays I got files in my files.\"", "-- Gucci Mane, probably", theme);
    ...
    if (engine->isRecording()) {
        ...
        if (pending) {
            ...
            ImGui::TextWrapped("Next player death will be recorded as intentional. Playback will "
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped(
            "Inserts a TPS change action at the current frame. Change TPS in the Tools tab first.");
        ImGui::PopStyleColor();
        ...
        if (ImGui::BeginPopupModal("SaveFrameWindows",
            ...
            ImGui::TextUnformatted(
                "Calculate frame windows for this macro? This replays the macro and simulates each "
            ...
            ImGui::TextUnformatted(
                ...
                "ground-truth-forced), but each click's tested timing shifts still fall back to "
    ...
    float listH =
        std::max(80.f, std::min(200.f, (float)engine->storedMacros.size() * 28.f + listPadY * 2));
    ...
    if (engine->storedMacros.empty()) {
        ...
        const char* emptyMsg = (activeTheme == THEME_TOOSII) ? "No saved replays -- get to work"
    ...
    for (const auto& mn : macroCopy) {
        if (!matchesFilter(mn))
            continue;
        bool isSel = (!engine->replay.m_actionAtom.m_actions.empty() && !engine->isRecording() &&
                      engine->replayName == mn);
    ...
    if (ImGui::BeginPopupModal("MacroActions",
        ...
        if (Widgets::StyledButton("Rename##ar", ImVec2(aw, 30), theme, anim, 6.f)) {
            ...
            strncpy(
                replayRenameBuffer, replayActionMacroName.c_str(), sizeof(replayRenameBuffer) - 1);
    ...
    if (ImGui::BeginPopupModal("RenameReplay",
        ...
        if (confirm || submitted) {
            ...
            if (renameStoredReplay(
                    replayRenameOriginalName, replayRenameBuffer, renamedTo, replayRenameError)) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped(
        "Merge appends B after A using the Compare fields above. Gap = extra frames between them.");
    ImGui::PopStyleColor();
    ImGui::Text("Gap");
    ImGui::SameLine(70);
    ImGui::SetNextItemWidth(-1);
    ...
    if (!engine->replay.m_actionAtom.m_actions.empty() && engine->isPlaying() && PlayLayer::get()) {
        ...
        if (engine->replay.m_pathSamples.empty()) {
            ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
            ImGui::TextWrapped("No path data for this macro -- only recorded going forward, not "
        ...
        if (engine->showMacroPath && !engine->replay.m_pathSamples.empty()) {
            ...
            ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
            ImGui::TextWrapped("Line shows the recorded path. Squares mark clicks (and releases in "
...
void MenuInterface::drawToolsTab() {
    ...
    if (auto* toolsCustom = getActiveCustomTheme())
        ...
        Widgets::GucciQuote(
            "\"I don't need speedhack. I've had worse rides.\"", "-- Tony Romo, probably", theme);
    else if (activeTheme == THEME_GRIZZLEY)
        Widgets::GucciQuote(
            "\"I don't need speedhack. I move different.\"", "-- Tee Grizzley, probably", theme);
    else if (activeTheme == THEME_REDKINGDOM)
    ...
    if (Widgets::ModuleCardBegin("Lock Delta",
                                 ...
                                 anim)) {
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("Forces exact 1/TPS every step. The old Performance mode (batching "
    ...
    if (Widgets::ModuleCardBegin(
        ...
        if (engine->noclipDeathFlash) {
            float col[3] = {
                engine->noclipDeathColorR, engine->noclipDeathColorG, engine->noclipDeathColorB};
            if (ImGui::ColorEdit3(
...
static geode::Task<int> importFwAssetFolderTask() {
    auto pickResult =
        co_await geode::utils::file::pick(geode::utils::file::PickMode::OpenFolder,
...
void MenuInterface::drawFrameWindowsTab() {
    ...
    Widgets::SectionHeader("Frame Window Tracker", theme);
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped(
            locked ? "Locked while Calculate is running -- finish or cancel the current run to "
                     "switch algorithms."
                   : "Time-Based (default): faster, watches the shifted click survive on its own "
                     "-- good for most levels. Recovery Range: Juice's original algorithm, revived "
                     "-- also checks whether the NEXT click's timing could shift slightly to still "
    ...
    ImGui::TextWrapped(
        engine->isRecording()
            ? "Show Live and Show Legend are locked off while recording -- both read state that "
              "doesn't exist yet mid-recording. Available again once recording stops."
            : "Juice's idea: a running tally in the top-left corner, like the frame-window counter "
              "overlays in some GD YouTube videos -- how many of the clicks reached so far landed "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Marker stroke thickness for the concentric double-ring style. Only affects "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("In Robot mode, a release right after clicking a non-dash orb isn't treated "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped(
        "A shift survives if it stays alive through roughly how long the original macro takes to "
        "reach the next input, MINUS this many frames of slack (a shorter survival still counts as "
        "a pass -- it never needs to survive longer than the original gap). The next input itself "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Juice's request: surviving isn't proof a shift actually worked -- the "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Off (default): stop expanding a direction the moment one shift fails "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Pauses briefly after every individual shift test and drops a green/red "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Juice's position-lag report: run Calculate once with this off, once with "
    ...
    if (engine->fwDebugMode &&
        Widgets::StyledSliderInt("Debug Pause (ticks)", &engine->fwDebugSlowdown, 1, 120, theme))
    ...
    if (engine->fwHasData) {
        ...
        ImGui::TextWrapped("No analysis yet. Save a macro while in the level and choose Calculate "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Set or override a click's window by hand -- for clicks Calculate hasn't "
    ...
    if (engine->fwAnalyzing) {
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped(
            "Unavailable while Calculate is running -- it's actively reshaping the macro's action "
    ...
    ImGui::TextWrapped(
        "Sound: leave blank for the built-in default chime, type a filename for custom, or 'none' "
    ...
    for (size_t ti = 0; ti < engine->fwTiers.size(); ++ti) {
        ...
        ImGui::InputTextWithHint(
            "##legendgroup",
            "Legend group (optional -- e.g. make lo=hi=5 and lo=hi=6 both '5-6' to customize each "
        ...
        if (ImGui::TreeNodeEx("Pulse Effects", ImGuiTreeNodeFlags_None)) {
            ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
            ImGui::TextWrapped("Fades from the marker/text's normal color to the pulse color, "
    ...
    if (!engine->fwTiers.empty()) {
        if (Widgets::StyledButton("Save Tiers", ImVec2(-1, 24), theme, anim, 6.f)) {
            ...
            for (auto& t : engine->fwTiers) {
                enc +=
                    std::to_string(t.lo) + "|" + std::to_string(t.hi) + "|" + t.imageFile + "|" +
                    t.soundFile + "|" + std::to_string(t.r) + "|" + std::to_string(t.g) + "|" +
                    std::to_string(t.b) + "|" + std::to_string((int)t.shape) + "|" +
                    std::to_string(t.polygonSides) + "|" + std::to_string(t.polygonCornerRadius) +
                    ...
                    std::to_string(t.markerPulseFadeIn) + "|" + std::to_string(t.markerPulseHold) +
                    "|" + std::to_string(t.markerPulseFadeOut) + "|" +
                    (t.textPulseEnabled ? "1" : "0") + "|" + std::to_string(t.textPulseColor[0]) +
                    "|" + std::to_string(t.textPulseColor[1]) + "|" +
                    std::to_string(t.textPulseColor[2]) + "|" + std::to_string(t.textPulseFadeIn) +
...
void MenuInterface::drawRenderTab() {
    ...
    if (!SLRenderer::get()->isFFmpegLoaded()) {
        ...
        ImGui::TextWrapped(
            ...
            "swscale-9.dll, avcodec-62.dll, avformat-62.dll, avfilter-11.dll, avdevice-62.dll) and "
    ...
    if (renderIncludeAudio) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("Off: one combined audio track, same as always. On: four tracks in the "
...
void MenuInterface::drawClicksTab() {
    ...
    if (Widgets::ModuleCard(
            "Click Sounds", "Play click and release sounds on input", &csm->enabled, theme, anim))
...
void MenuInterface::drawSettingsTab() {
    ...
    ImGui::TextWrapped(
        ...
        "noclip, and a few other essentials, small enough to leave open while actually playing.");
    ImGui::PopStyleColor();
    ImGui::Dummy(ImVec2(0, 8));
    ...
    Widgets::SectionHeader("Diagnostics", theme);
    {
        ...
        ImGui::TextWrapped(
            ...
            "call site, to the mod log. Flip on right before reproducing (an MH step, a release "
    ...
    Widgets::SectionHeader("Fun", theme);
    {
        ...
        if (brrr->hasFile())
            ImGui::TextWrapped(
                "Loops the audio file in your BRRRR folder (overriding the bundled one) and makes "
                "the whole menu bounce. Doesn't touch the Jupiter tab.");
        else
            ImGui::TextWrapped("Loops the bundled BRRRR track and makes the whole menu bounce. "
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("Reads the track's actual bass energy live and shakes/flickers the menu "
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("How far the menu fades out on hard hits. 0 turns the flicker off "
...
void MenuInterface::drawMoreHacksTab() {
    ...
    if (Widgets::ToggleSwitch(
            "Force Platformer Controls (experimental)", &engine->hackForcePlatformer, theme, anim))
        mod->setSavedValue("hack_force_platformer", engine->hackForcePlatformer);
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped(
        "Hide Attempts, No Death Flash, Auto Retry, and Instant Respawn are wired. Hide Percentage "
...
void MenuInterface::drawIndicatorsTab() {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("A marker on the player that turns green when the next click keeps you "
    ...
    {
        float col[3] = {
            engine->indicatorSafeColorR, engine->indicatorSafeColorG, engine->indicatorSafeColorB};
        if (ImGui::ColorEdit3(
                "##indSafeC", col, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel)) {
    ...
    ImGui::SameLine();
    {
        ...
        if (ImGui::ColorEdit3(
                "##indDangerC", col, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel)) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped(
        "Calibration currently measures and stores your lead/jitter per gamemode. It does not yet "
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("On-screen readout of how many of your real clicks matched the indicator's "
...
void MenuInterface::drawJupiterClickTrainerPage() {
    ...
    Widgets::SectionHeader("Click Deviation", theme);
    {
        ...
        if (engine->jupiterMacro.clickIntervalsSec.empty()) {
            ...
            ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
            ImGui::TextWrapped("Play the level yourself (not bot playback) to compare your clicks "
                               "against the macro's.");
            ImGui::PopStyleColor();
        } else {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Plays while actually on Jupiter My Favourite, seeked to match your current "
    ...
    if (engine->jupiterScrubActive) {
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("Ghosts freeze at this position in the level instead of following live "
...
void MenuInterface::drawJupiterTab() {
    ...
    std::string currentLevel =
        (pl && pl->m_level) ? std::string(pl->m_level->m_levelName) : engine->loadedMacroLevelName;
    ...
    if (engine->replay.m_pathSamples.empty()) {
        ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
        ImGui::TextWrapped("No path data yet for the loaded macro. Load your converted TCBot macro "
        ...
        if (segs.size() < 2) {
            ...
            ImGui::TextWrapped(
                !validRange
                    ...
                    : "The first time you reach the start segment, a real practice checkpoint gets "
                      ...
                      "respawns you there automatically, same as normal practice mode. Only places "
                      "one per enable, so it won't pile up checkpoints or touch any you've placed "
                      "yourself elsewhere.");
            ImGui::PopStyleColor();
            ...
            if (engine->jupiterLoopEnabled && validRange && pl && pl->m_player1) {
                ...
                if (px < startX + 5.f) {
                    ...
                    if (!checkpointPlaced) {
                        ...
                        pl->markCheckpoint();
                        Notification::create("Loop checkpoint placed", NotificationIcon::Success)
                            ->show();
                    }
    ...
    ImGui::Dummy(ImVec2(0, 8));
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Rehearsal mode (scrub playback, segment looping) isn't built yet -- that's "
...
void MenuInterface::drawTrainerTab() {
    ...
    if (fontHeading)
        ImGui::PopFont();
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Same toolset as the JMF tab -- Click Trainer, Ghosts, Segments, Stats, "
    ...
    ImGui::SetNextItemWidth(-1);
    ImGui::InputTextWithHint(
        "##trainerMacroSearch", "Search macros...", trainerMacroFilter, sizeof(trainerMacroFilter));
        ...
        ImGui::TextWrapped(currentLevel.empty()
                               ? ("No level loaded. Enter \"" + engine->trainerMacro.levelName +
                                  "\" to activate Stats/Ghost/Music for this macro.")
                                     .c_str()
                               : ("Currently on \"" + currentLevel + "\" -- this macro is for \"" +
    ...
    Widgets::SectionHeader("Segments", theme);
    ImGui::PushStyleColor(ImGuiCol_Text, theme.textSecondary);
    ImGui::TextWrapped("Personal landmarks -- name the hard parts. Doesn't jump you there, just "
...

    {
        ...
        for (int i = 0; i < (int)segs.size(); i++) {
            ...
            if (noteEditIdxT == i) {
                ImGui::SetNextItemWidth(-1);
                if (ImGui::InputTextWithHint(
                        "##trainerSegNote", "note for this segment", noteBufT, sizeof(noteBufT))) {
...
void MenuInterface::drawCreditsTab() {
    ...
    if (auto* creditsQuoteCustom = getActiveCustomTheme())
        Widgets::GucciQuote(creditsQuoteCustom->quoteCredits.text.c_str(),
             ...
             activeTheme == THEME_TOOSII_SACSTATE)
        Widgets::GucciQuote("\"Every click is a catch. I don't drop nothing. Not even frames.\"",
        ...
        Widgets::GucciQuote(
            "\"Every frame is a bucket. 83 of them. BITCH IM KOBE!!!\"", "-- Bam Adebayo", theme);
    else if (activeTheme == THEME_SEXYY)
...
void MenuInterface::loadRenderSettings() {
    ...
    auto rv =
        loadSV<std::string>(mod, "render_video_args", "colorspace=all=bt709:iall=bt470bg:fast=1");
...
void MenuInterface::loadSettings() {
    ...
    theme.activePreset = std::clamp(
        mod->getSavedValue<int>("theme_active_preset", 0), 0, ThemeEngine::getPresetCount() - 1);
...
void displayAccuracyHUD() {
    ...
    int pct =
        engine->accuracyTotalClicks > 0
            ? (int)((float)engine->accuracyGoodClicks / (float)engine->accuracyTotalClicks * 100.f +
                    0.5f)
            : 100;
    ...
    if (ui->fontBody)
        ImGui::PushFont(ui->fontBody);
    ImGui::Text(
        "Accuracy: %d%%   Streak: %d (Best: %d)", pct, engine->currentStreak, engine->bestStreak);
    if (ui->fontBody)
        ImGui::PopFont();
    ImGui::End();
}
