...
namespace {
    ...
    std::vector<CCPoint> squareVertices(CCPoint center, float half) {
        return {ccp(center.x - half, center.y - half),
                ccp(center.x + half, center.y - half),
                ccp(center.x + half, center.y + half),
                ccp(center.x - half, center.y + half)};
    }
...
public:
    ...
    void attach(PlayLayer* pl) {
        if (m_lineNode || !pl || !pl->m_objectLayer)
            return;
    ...
    void detach() {
        if (m_lineNode) {
            m_lineNode->removeFromParent();
            m_lineNode = nullptr;
        }
        if (m_markerNode) {
            m_markerNode->removeFromParent();
            m_markerNode = nullptr;
        }
        m_builtForCount = SIZE_MAX;
...
private:
    void rebuild(const std::vector<MacroPathSample>& samples, float revealX) {
        ...
        auto drawMarker = [&](CCPoint at) {
            ...
            m_markerNode->drawPolygon(verts.data(),
                                      verts.size(),
        ...
        for (auto const& a : gb->replay.m_actionAtom.m_actions) {
            if (a.m_type != gb::ActionType::Jump)
                continue;
