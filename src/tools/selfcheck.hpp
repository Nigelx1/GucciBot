    ...
    namespace gbcheck {
        ...
        inline void run(int expectedMidhooks,
...

            {
                ...
                add("Midhooks installed",
                    ok,
                    std::to_string(got) + "/" + std::to_string(expectedMidhooks));
                if (ok)
                    geode::log::info(
                        "[GucciBot]  [OK]   Midhooks: {}/{} installed", got, expectedMidhooks);
                else
...

            {
                ...
                if (ok)
                    geode::log::info(
                        "[GucciBot]  [OK]   Patches: {}/{} applied", got, expectedPatches);
                else
...

            {
                ...
                if (ok)
                    geode::log::info(
                        "[GucciBot]  [OK]   Formats: GBR6 v{}, BRR v{}", gbr6Version, brrVersion);
                else
            ...
            if (g_failCount == 0)
                geode::log::info(
                    "[GucciBot] self-check PASSED ({} checks). Build {} healthy. Brrr.",
                    g_passCount,
                    modVersion);
            else
