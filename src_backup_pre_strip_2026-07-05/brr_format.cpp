...
static std::string readString(std::vector<uint8_t> const& data, size_t& position) {
    ...
    if (position + length > data.size()) {
        throw std::runtime_error("BRR: unexpected end of string data");
    }
...
static uint32_t readVarint(std::vector<uint8_t> const& data, size_t& position) {
    ...
    while (position < data.size()) {
        ...
        shift += 7;
        if (shift >= 35) {
            throw std::runtime_error("BRR: varint too large");
        }
    }

    throw std::runtime_error("BRR: unexpected end of varint");
}
...
// Timed-accuracy modes store a per-input sub-step offset float
static bool usesTimedAccuracy(AccuracyMode mode) {
...
static void writeHeader(std::vector<uint8_t>& buffer, BRRMacro const& macro) {
    buffer.push_back('B');
    buffer.push_back('R');
    buffer.push_back('R');
    buffer.push_back('\0');
...
void BRRMacro::recordAction(int tick, int button, bool player2, bool pressed, float offset) {
    BRRInput input;
    input.tick = tick;
...
void BRRMacro::truncateAfter(int tick) {
...
std::vector<uint8_t> BRRMacro::serialize() const {
    ...
    for (auto const& input : inputs) {
        // Reset delta at attempt boundary (tick goes backwards)
    ...
    // Write death frames into payload (before compression)
    if (!deathFrames.empty()) {
        ...
        // Write attempt start ticks right after death frames
...
static BRRMacro* deserializeCompressedPayload(
    ...
    try {
        ...
        bool hasTimedOffsets = (flags & (BRR_FLAG_ACCURACY_CBS | BRR_FLAG_ACCURACY_CBF)) != 0;
        ...
        // Read death frames if present (inside try, same scope as payload/payloadPosition)
        if ((flags & BRR_FLAG_HAS_DEATH_FRAMES) != 0 &&
            ...
            // Read attempt start ticks (written right after death frames)
...
BRRMacro* BRRMacro::deserialize(std::vector<uint8_t> const& data) {
    if (data.size() < 14) {
        return nullptr;
    }

    if (data[0] != 'B' || data[1] != 'R' || data[2] != 'R' || data[3] != '\0') {
        return nullptr;
    }
    ...
    auto* macro = new BRRMacro();
    if ((flags & BRR_FLAG_ACCURACY_CBF) != 0) {
        macro->accuracyMode = AccuracyMode::CBF;
    } else if ((flags & BRR_FLAG_ACCURACY_CBS) != 0) {
        macro->accuracyMode = AccuracyMode::CBS;
    } else {
        macro->accuracyMode = AccuracyMode::Vanilla;
    }
    macro->recordedFromStartPos = (flags & BRR_FLAG_FROM_START_POS) != 0;
    macro->platformerMode = (flags & BRR_FLAG_PLATFORMER) != 0;
    macro->twoPlayerMode = (flags & BRR_FLAG_TWO_PLAYER) != 0;
    macro->rngLocked = (flags & BRR_FLAG_RNG_LOCKED) != 0;
...
namespace ReplayStorage {
    ...
    static std::string makeUniqueReplayName(std::string const& wanted, std::string const& persistedName) {
        ...
        // Re-saving under our own persisted name overwrites in place
...
void BRRMacro::persist() {
    ...
    std::string ext = getThemeExtension();
    std::ofstream output(directory / (name + ext), std::ios::binary);
...
BRRMacro* BRRMacro::loadFromDisk(std::string const& filename) {
    ...
    // Try all known native extensions
    std::filesystem::path path;
    for (auto& ext : std::initializer_list<const char*>{".brrr", ".toosii", ".ja", ".giddey", ".bam", ".sexyy"}) {
        auto candidate = directory / (filename + ext);
        if (std::filesystem::exists(candidate)) { path = candidate; break; }
    }
    if (path.empty()) {
        // Try bare filename (legacy)
...
std::vector<MacroAction> BRRMacro::toMacroActions() const {
