...
// replay.hpp not needed in v7

// Types that were in replay.hpp — fully restored for v8
#include <array>
...
struct MacroAction {
    ...
    MacroAction() = default;
    // Order matches BRRMacro::toMacroActions(): (tick, button, player2, pressed, offset)
...
public:
    ...
    std::vector<int32_t> attemptStartTicks; // actual tick when each attempt starts (for playback offset) // intentional death frame numbers
    ...
    // Check if tick matches a specific death frame by index
    ...
    // Check if tick matches ANY death frame (legacy use)
