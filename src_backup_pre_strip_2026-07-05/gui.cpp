...
// -----------------------------------------------------------------------
// Theme presets - GucciBot and ToosiiBot first, then extras
// -----------------------------------------------------------------------
static const ThemePreset kThemePresets[]={
    ...
    {"ToosiiBot (LSU)",
     ...
     5.f,0.96f},
    // 2: ToosiiBot Syracuse - orange + navy
    {"ToosiiBot (Syracuse)",
     ImVec4(0.961f,0.404f,0.031f,1.f),  // Syracuse orange
     ImVec4(0.027f,0.043f,0.114f,0.96f), // navy bg
     ImVec4(0.055f,0.082f,0.188f,1.f),
     ImVec4(0.960f,0.940f,0.920f,1.f),
     ImVec4(0.600f,0.500f,0.400f,1.f),
     5.f,0.96f},
    // 3: ToosiiBot Sac State - dark green + gold/white
    {"ToosiiBot (Sac State)",
     ImVec4(0.918f,0.878f,0.820f,1.f),  // white/gold accent
     ImVec4(0.016f,0.188f,0.094f,0.96f), // dark green bg
     ImVec4(0.024f,0.251f,0.125f,1.f),
     ImVec4(0.940f,0.960f,0.940f,1.f),
     ImVec4(0.500f,0.650f,0.520f,1.f),
     5.f,0.96f},
    // 4: JaBot - navy + light blue
    {"JaBot",
     ImVec4(0.420f,0.784f,0.953f,1.f),  // light blue accent
     ImVec4(0.027f,0.078f,0.200f,0.96f), // navy bg
     ImVec4(0.047f,0.118f,0.275f,1.f),
     ImVec4(0.920f,0.950f,0.980f,1.f),
     ImVec4(0.400f,0.540f,0.720f,1.f),
     5.f,0.96f},
    // 5: GiddeyBot - red + white
    {"GiddeyBot",
     ImVec4(0.871f,0.122f,0.122f,1.f),  // red accent
     ImVec4(0.098f,0.039f,0.039f,0.96f), // dark red bg
     ImVec4(0.157f,0.063f,0.063f,1.f),
     ImVec4(0.980f,0.960f,0.960f,1.f),
     ImVec4(0.620f,0.420f,0.420f,1.f),
     5.f,0.96f},
    // 6: BamBot - Miami Heat (navy-black bg, Heat red accent, white text)
    {"BamBot",
     ImVec4(0.878f,0.067f,0.153f,1.f),  // Heat red
     ImVec4(0.047f,0.027f,0.071f,0.96f), // Heat navy-black
     ImVec4(0.078f,0.043f,0.114f,1.f),
     ImVec4(0.980f,0.980f,0.980f,1.f),
     ImVec4(0.600f,0.400f,0.460f,1.f),
     5.f,0.97f},
    // 7: SexyyBot - hot pink + deep purple-black
    {"SexyyBot",
     ImVec4(0.910f,0.004f,0.580f,1.f),  // hot pink accent
     ImVec4(0.063f,0.016f,0.094f,0.97f), // deep purple-black bg
     ImVec4(0.102f,0.027f,0.149f,1.f),   // slightly lighter purple card
     ImVec4(0.990f,0.950f,0.980f,1.f),   // near-white pink-tinted text
     ImVec4(0.580f,0.340f,0.520f,1.f),   // muted pink secondary
     5.f,0.97f},
};
...
// -----------------------------------------------------------------------
// processHotkeys (empty - handled in keybinds.cpp via CCKeyboardDispatcher)
// -----------------------------------------------------------------------
// GucciBot 8.0 engine — hotkeys handled in keybinds.cpp
// fastPlayback is now a real field on GucciEngine
...
bool ToggleSwitch(const char* label,bool* value,ThemeEngine& theme,AnimationState& anim){
    ...
    // Track is a pill: width=36, height=16. Rounded caps = height/2 = 8px radius.
    // Knob radius = 6. Knob center travels from cap-center to cap-center:
    //   left  = trackMin.x + trackH/2       = trackMin.x + 8
    //   right = trackMin.x + trackW - trackH/2 = trackMin.x + 28
...
bool StyledSliderInt(const char* label,int* v,int lo,int hi,ThemeEngine& theme){
    // Draw the label as its own line first — a full-width slider (width -1)
    // pushes ImGui's built-in right-side label off-screen, which is why the
    // frame-window sliders showed no labels. Hide the slider's own label with ##.
...
bool ModuleCard(const char* name,const char* desc,bool* enabled,
    ...
    // toggle on right - same cap-aware math as ToggleSwitch
...
void MenuInterface::drawTitleBar(){
    ...
    const char* botName=
        ...
        (activeTheme==THEME_SEXYY)?"SexyyBot":"GucciBot";
...
void MenuInterface::drawTabBar(){
    // Main top tabs: Macro | Render | Clicks | Autoclicker | HUD | Settings | Credits
    ImDrawList* dl=ImGui::GetWindowDrawList();
    ImVec2 pos=ImGui::GetCursorScreenPos();
...
void MenuInterface::drawStatusBar(){
    ...
    // Brrr / First down watermark on right
    const char* brand=
        (activeTheme==THEME_TOOSII||activeTheme==THEME_TOOSII_SYRACUSE||activeTheme==THEME_TOOSII_SACSTATE)?
            "Open!":
        ...
        (activeTheme==THEME_SEXYY)?"Skee yee.":"Brrr.";
...
void MenuInterface::drawMainWindow(){
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_Alpha,t);
    // Allow dragging - no NoMove flag
    ImGui::Begin("##GucciBot",nullptr,
        ImGuiWindowFlags_NoDecoration|ImGuiWindowFlags_NoScrollbar|
        ImGuiWindowFlags_NoScrollWithMouse|ImGuiWindowFlags_NoTitleBar);
    // Draw drag handle - thin strip at top that glows on hover
    {
        ...
        // Animated shimmer lines like TR1.3
    ...
    float contentH=ws.y-52-40-14;  // titlebar + statusbar + padding
...
// -----------------------------------------------------------------------
// drawMegaHackWindow — v8.2: MegaHack-v7-style shell. Same tabs, same state,
// same drawTabContent() — only the chrome changes: left rail instead of the
// top tab bar, slate panels, compact header. Fully switchable from Settings.
// -----------------------------------------------------------------------
void MenuInterface::drawMegaHackWindow(){
    ...
    if(fontHeading)ImGui::PopFont();
    // rail entries — shared activeTab/switchTab: state identical across skins
    const char* names[]={"Macro","Render","Clicks","Autoclicker","Hacks","HUD","Settings","Credits"};
...
void MenuInterface::drawReplayTab(){
    ...
    if(Widgets::PillButton("Disable",engine->isIdle(),pillW,theme,anim)){
        if(engine->isRecording()){
            // Auto-save recording on disable
        }
    ...
    if(Widgets::PillButton("Record",engine->isRecording(),pillW,theme,anim)){
        if(engine->isRecording()){
            ...
            // v8.1: RESUME RECORDING — pressing Record mid-playback appends
            // from the current frame instead of wiping the macro
    ...
    // Format select popup is removed in v5 — always records to BRR/slc format
    // The themed extension is applied at save time based on active theme
    if(ImGui::BeginPopup("##FmtSel",ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoResize)){
        drawPopupChrome(*this,"Select Format");
        float bw=110.f;
        // Label shows the active theme's extension
    ...
    if(engine->isRecording()){
        ...
        if(Widgets::StyledButton("Save Macro",ImVec2(bw,30),theme,anim)){
            if(!engine->replay.m_actionAtom.m_actions.empty()){
                ...
                ImGui::OpenPopup("SaveFrameWindows");  // v8.8
            }
        }
        // v8.8: offer frame-window analysis after a save
        if(ImGui::BeginPopupModal("SaveFrameWindows",nullptr,ImGuiWindowFlags_AlwaysAutoResize|ImGuiWindowFlags_NoTitleBar)){
            ...
            ImGui::PushStyleColor(ImGuiCol_Text,theme.textSecondary);
            // AlwaysAutoResize + TextWrapped collapse to ~5 chars/line (window width
            // depends on the wrapped text, which depends on window width). Wrap at a
            // fixed position instead so the popup has a definite width.
    ...
    for(const auto& mn:macroCopy){
        if(!matchesFilter(mn))continue;  // v8.5: search filter (before any PushID)
        ...
        float rightReserved=xBtnW+dotsBtnW+btnGap+listPadX; // space for both buttons
        std::string rowLbl=mn;
        float accW = 0.f; // v6: no CBF/CBS tags
        // Determine tag string for width calculation
        ...
        // Selectable uses macro name as ID via PushID scope - use ##row to avoid label conflict
        ...
        if(rowDbl||rowRC){replayActionMacroName=mn;replayActionPopupRequested=true;}
            ...
            // v5: load macro via replay system (always BRR/slc format)
            {
                // Try all themed extensions
                std::string extFound;
                ...
                if(!extFound.empty()){
                    ...
                    // Legacy macro — prompt to convert
                    engine->convertToBRR(mn);
                    engine->replayName=mn;
                }
            }
        }  // v8.0.1: close load branch HERE — row drawing + PopID must run every frame
        ...
        // Tags - drawn right-to-left from before the buttons
        ...
        ImGui::PopStyleVar();
        ImGui::PopID();
    } // end for(mn:macroCopy)
    ImGui::EndChild();
    ImGui::PopStyleVar(2);ImGui::PopStyleColor();
    // Macro action popup (right-click)
...
void MenuInterface::drawToolsTab(){
    ...
    // ── Noclip accuracy display toggle ───────────────────────────────────────
    ImGui::Dummy(ImVec2(0,4));
...
void MenuInterface::drawHacksTab(){
    ...
    if(Widgets::ModuleCardBegin("Noclip",
        ...
        // Accuracy display (hidden toggle, off by default)
    ...
    // Max window can never exceed 2*sweep (a click can't be looser than the full
    // sweep both ways), so cap the slider + value there. With scoop 12 -> max 24.
    ...
    // v10.1: live analysis progress — turns the old black-box Calculate into
    // visible feedback. Shows a bar + stage + click counter while it runs.
    ...
    // v8.11: frame-gap tiers — map ranges to a PNG marker + a sound.
    ImGui::Dummy(ImVec2(0,6));
    ImGui::PushStyleColor(ImGuiCol_Text,theme.textSecondary);
...
void MenuInterface::drawRenderTab(){
    ...
    // Preset name input + save/load buttons
    ...
    if(Widgets::StyledButton("Save##preset",ImVec2(76,0),theme,anim)){
        // Save all render settings under this preset name key
    ...
    // Output folder - full width editable field
    ...
    ImGui::Dummy(ImVec2(0,4));
    // v10.3 (render port Stage 2): Render button drives the new SLRenderer
    // (Silicate FFmpeg-library engine), not the legacy TTR ffmpeg.exe renderer.
    ...
    // v8.4: one-tap quality presets — resolution + fps + bitrate together
    {
    ...
    if(ImGui::BeginCombo("##vCodecCombo",renderCodecBuf[0]?renderCodecBuf:"Select...")){
        ...
        for(int i=0;i<vCodecCount;i++){
            ...
            if(sel)ImGui::SetItemDefaultFocus();}
        ImGui::Separator();
        // Also allow typing a custom codec
        ImGui::PushStyleColor(ImGuiCol_Text,theme.textSecondary);
    ...
    // ── Scroll Speed Fix (also in Tools, mirrored here for render workflow) ──
    ImGui::Dummy(ImVec2(0,8));
...
void MenuInterface::drawSettingsTab(){
    ...
    // v8.10: Diagnostics / self-check panel
    Widgets::SectionHeader("Diagnostics",theme);
    {
        // Build label — lets Juice confirm which build he's testing.
        ImGui::PushStyleColor(ImGuiCol_Text,theme.getAccent());
    ...
    // Theme preset - 2-column grid, taller buttons so text fits
    ...
    for(int i=0;i<pc;i++){
        ...
        if(Widgets::PillButton(presets[i].name,active,colW,theme,anim)){
            ...
            saveSettings();}
        if(i%2==0&&i+1>=pc)ImGui::Dummy(ImVec2(0,0)); // end odd row
    }
...
// -----------------------------------------------------------------------
// drawMoreHacksTab — v8.12: dedicated MegaHack-style hacks (safe batch)
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// drawCreditsTab - big Gucci Mane Fan hero + credits list
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// drawHudTab – v4.0: HUD label system ported from Silicate
// -----------------------------------------------------------------------
...
// -----------------------------------------------------------------------
// saveSettings / loadRenderSettings / loadSettings
// -----------------------------------------------------------------------
...
void MenuInterface::loadSettings(){
    ...
    // eng_brr_mode: always true in v6
    ...
    if(eng->fwMaxWindow > 2*eng->fwSweepRange) eng->fwMaxWindow = 2*eng->fwSweepRange;  // can't exceed full sweep
    ...
    // v8.11: decode saved frame-window tiers (lo|hi|img|snd|r|g|b; per tier)
    {
...
// -----------------------------------------------------------------------
// drawInterface / initialize / overlay
// -----------------------------------------------------------------------
void MenuInterface::drawInterface(){
    auto* engine=GucciEngine::get();
    if(!setupComplete)return;
    anim.update(ImGui::GetIO().DeltaTime);
    // When close animation finishes, mark as hidden so keybind can reopen
...
void displayOverlayBranding(){
    ...
    // Only show watermark when menu is open
    if(!ui->shown)return;
    auto* vp=ImGui::GetMainViewport();
...
// v8.2: render progress HUD — the game view looks frozen during renders by
// design (frames go to the capture FBO), so this overlay is the heartbeat:
// pulsing REC, frames written, captured level time, target fps.
...
// v10.1: gameplay HUD — the HUD tab had all the toggles and saved/loaded its
// config, but nothing ever drew it on screen. This renders the enabled fields
// (frame, TPS, position, velocity, rotation, state) as an ImGui overlay window,
// anchored per the user's choice. Modeled on displayRenderHUD.
void displayGameplayHUD(){
    ...
    // Build the lines from the enabled fields.
