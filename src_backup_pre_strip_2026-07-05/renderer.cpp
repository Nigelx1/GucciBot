...
#include "brr_format.hpp"  // v8.3: full MacroAction type — the click-mux code constructs them
...
static std::string resolveFfmpegExecutable(std::filesystem::path const& configuredPath) {
    ...
    // Check mod save dir (where Geode unpacks resources/ffmpeg.exe)
    {
        std::error_code ec;
        ...
        // Also check resources dir directly
...
int Renderer::getCurrentFrame() const {
    auto* engine = GucciEngine::get();
    return static_cast<int>(engine->updater.getFrame());
}

float Renderer::getTPS() const {
    return static_cast<float>(GucciEngine::get()->updater.m_tps);
}
...
void Renderer::start() {
    ...
    // v8.1: automatic color fix — without an explicit colorspace conversion,
    // GD renders come out washed out (bt470bg source vs the bt709 players
    // assume). Injected unless the user already set their own colorspace.
    if (mod->getSavedValue<bool>("render_color_fix", true)) {
        if (videoArgs.find("colorspace=") == std::string::npos &&
            ...
            videoArgs = videoArgs.empty() ? colorFix : (colorFix + "," + videoArgs);
        }
    }

    // v8.0.1: dedicated output pixel format (e.g. yuv444p). Authoritative on
    // both encoder paths: exe gets "-pix_fmt X" appended last (wins over any
    // -pix_fmt in extra args, which we strip) plus a format filter; the
    // FFmpeg-API path gets the format filter via m_colorspaceFilters.
    ...
    std::error_code rfec;
    if (!std::filesystem::exists(renderFolder, rfec))
    ...
    // v10.3 (P2, render-from-0 REAL FIX): the user renders while PAUSED (to pin
    // frame 0), but the capture hijack requires !m_isPaused — so a paused render
    // captured nothing (262 bytes; this is why -d and -f "failed"). So: unpause
    // on render start, THEN reset to frame 0. resetLevel() (not the
    // resetLevelFromStart that broke -d) is the respawn-style reset that keeps the
    // level started/playing (m_started stays true) and rewinds inputIndex so the
    // macro replays from 0.
    if (engine->isPlaying() && !engine->replay.m_actionAtom.empty()) {
        pl->m_isPaused = false;  // capture hijack needs the level NOT paused
        engine->practiceFix.m_savedCheckpoints.clear();  // start from 0, not a checkpoint
        pl->resetLevel();  // respawn-style reset to frame 0, keeps m_started true
    }

    dontRender = false;  // v8.0.1: was set true and never cleared (TTR clears it
                         // in resetLevel) — every handleRecording call bailed out
    recording = true;
    levelFinished = false;
    timeAfter = 0.f;
    finishFrame = 0;
    ...
    renderTex.begin();
    changeRes(false);
    // resetSimulationTimingState (removed in v6)
    ...
    std::thread([this, songFile, songOffset, fadeIn, fadeOut, extension, bitrateApi, apiPixFmtFilter]() {
        ...
        settings.m_colorspaceFilters = apiPixFmtFilter;  // v8.0.1: output pix fmt on API path
        ...
        if (includeClickSounds) {
            ...
            // v8.1: click audio generation wired up — converts the live action
            // atom to MacroActions and mixes the generated click track into
            // the raw audio buffer (additive, extends the buffer if needed).
            if (hasInputs) {
...
// v8.6: capture a render's outcome for the GUI summary popup. Runs on the
// worker thread; only POD/string copies, and the GUI reads it on the main
// thread guarded by the `pending` flag, so no locking is needed.
...
void Renderer::handleRecording(PlayLayer* pl, int frame) {
    ...
    if (!pl->m_hasCompletedLevel || timeAfter < stopAfter) {
        ...
        // v10.3 (P2): force the very first capture at frame 0 so the render
        // includes the starter frame (e.g. a teleport portal at the level
        // start). Without this the first capture waits until levelTime >= 1/fps
        // and misses the opening frames ("starts a tiny bit later").
        if (!firstCaptureDone) {
            captureFrame();
...
class $modify(RenderPlayLayer, PlayLayer) {
    void resetLevel() {
        ...
        if (rnd.recording) {
            // v8.0.1: restored TTR's reset handshake — restarting the attempt
            // begins a clean capture from frame 0 (timing + dedup resynced)
            rnd.dontRender = false;
            rnd.lastFrame_t = 0;
            rnd.extra_t = 0;
            rnd.firstCaptureDone = false;
            rnd.renderedFrames.clear();
            rnd.levelFinished = false;
            rnd.timeAfter = 0.f;
            rnd.levelStartFrame = 0;
        }
    }
