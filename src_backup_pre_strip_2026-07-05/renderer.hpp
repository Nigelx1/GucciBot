...
public:
    ...
    // v8.6: last-render summary (set on the worker, shown by the GUI popup)
    struct LastRender {
        bool        pending = false;   // a fresh result the GUI hasn't shown yet
        bool        success = false;
        std::string path;
    ...
    double extra_t = 0;
    bool firstCaptureDone = false;  // v10.3 (P2): force a capture at frame 0 so the render includes the starter frame
